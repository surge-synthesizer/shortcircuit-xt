...
limiter::limiter(float *fp, int *ip) : filter(fp, 0, true, ip)
{
    ...
    parameter_count = 4;
    strcpy(ctrllabel[0], "input gain");
    ctrlmode[0] = cm_mod_decibel;
    strcpy(ctrllabel[1], "attack");
    ctrlmode[1] = cm_time1s;
    strcpy(ctrllabel[2], "release");
    ctrlmode[2] = cm_time1s;
    strcpy(ctrllabel[3], "initial");
    ctrlmode[3] = cm_percent;
    strcpy(ctrllabel[4], "SC freq");
    ctrlmode[4] = cm_frequency_audible;
    strcpy(ctrllabel[5], "SC gain");
    ctrlmode[5] = cm_decibel;
...
int limiter::get_ip_count() { return 2; }
const char *limiter::get_ip_label(int ip_id) { return "type"; }
int limiter::get_ip_entry_count(int ip_id) { return 2; }
...
void limiter::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR,
                             float pitch)
{
...
void limiter::process(float *datain, float *dataout, float pitch)
{
    ...
    if (iparam[0])
    {
        ...
        for (unsigned int k = 0; k < BLOCK_SIZE; k++)
        {
            ...
            __m128 mef = _mm_max_ss(_mm_load_ss(&ef), m128_one);
            if (iparam[1])
                x = _mm_mul_ss(x, _mm_rsqrt_ss(mef));
            else
                x = _mm_mul_ss(x, _mm_rcp_ss(mef));
...
gate::gate(float *fp) : filter(fp)
{
    ...
    strcpy(ctrllabel[2], "reduction");
    ctrlmode[2] = cm_decibel;
    strcpy(ctrllabel[3], "hysteresis");
...
void gate::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR,
                          float pitch)
{
    ...
    for (k = 0; k < BLOCK_SIZE; k++)
    {
        ...
        if (holdtime < 0)
            gate_state = false;
        if (!(onesampledelay[0] * datainL[k] > 0))
            gate_zc_sync[0] = gate_state;
        if (!(onesampledelay[1] * datainR[k] > 0))
...
void gate::process(float *datain, float *dataout, float pitch)
{
    ...
    for (k = 0; k < BLOCK_SIZE; k++)
    {
        ...
        if (holdtime < 0)
            gate_state = false;
        if (!(onesampledelay[0] * datain[k] > 0))
            gate_zc_sync[0] = gate_state;

        if (!gate_zc_sync[0])
            dataout[k] = reduction * datain[k];
...
stereotools::stereotools(float *fp, int *ip) : filter(fp, 0, true, ip)
{
    ...
    parameter_count = 2;
    strcpy(ctrllabel[0], "center");
    ctrlmode[0] = cm_percent_bipolar;
...
void stereotools::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR,
                                 float pitch)
{
...
int stereotools::get_ip_entry_count(int ip_id) { return 3; }
