...
LONG MiniDumper::TopLevelFilter(struct _EXCEPTION_POINTERS *pExceptionInfo)
{
    ...
    if (hDll)
    {
        ...
        if (pDump)
        {
            ...
            GetModuleFileName((HMODULE)hInstance, szDumpPath, _MAX_PATH);
            char *end = strrchr(szDumpPath, '\\');
            sprintf(end + 1, "sc2 minidump.dmp", szDumpPath);

            // ask the user if they want to save a dump file
            // if (::MessageBox( NULL, "Something bad happened in your program, would you like to
