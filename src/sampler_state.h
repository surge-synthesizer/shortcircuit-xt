...
{
	return (m == pm_forward_loop)/*||(m == pm_forward_loop_crossfade)*/||(m == pm_forward_loop_until_release)||(m == pm_forward_loop_bidirectional);
}

inline bool pm_has_slices(int m)
{
	return (m == pm_forward_hitpoints);
}
const char playmode_abberations[n_playmodes][16] = { "fwd","fwd_loop",/*"fwd_loop_cf",*/"fwd_loop_ur","fwd_loop_bi","fwd_shot","fwd_sliced","fwd_release"/*,"rev","rev_shot" */};
const char playmode_names[n_playmodes][32] = { "Standard","Loop",/*"Forward Loop-Crossfade",*/"Loop-Until-Release","Loop Bidirectional","Shot","Slice Keymapped","On Release"/*,"Reverse","Reverse Shot"*/};
...
const char output_abberations[n_output_types][16] = {	"Part bus",														
...
const char lfopreset_abberations[n_lfopresets][16] = {	
	"Load", "Clear", "Sine", "Triangle", "Square", 
	"Ramp up", "Ramp down", "Noise", "Noise m3", "Noise m5",
	"Tremolo <tri>", "Tremolo (sin)"
};
...
const char filter_abberations_beta1[14][6] = {
	"NONE",
	"LP2A",
	"HP2A",
	"BP2A",										
...
const char filter_descname[ft_num_types][32] = {
	"none",
	"Delay", "Reverb", "Chorus", "Phaser", "Rotary Speaker","Faux Stereo","Freqshift Flanger","Freqshift Delay",
	"LP/HP/BP/N 2-8P", "LP/HP/BP 2-4P", /*"Lowpass 2P",*/ /*"Lowpass 4P M",*/ "LP 1-4P MG",
	/*"Highpass 2P",*/ /*"Bandpass 2P (bw)", "Peak 2P", "Notch 2P (bw)",
	"Dual Bandpass 2P", "Dual Peak 2P",
	"LP/HP Serial", "LP/HP Parallel",*/ "EQ 2-band parametric",
	"EQ 6-band graphic", /*"morphEQ",*/ "Comb", "Lofi (digital)", "Distortion 1", "Clipper", "Slewer",
	"Stereo Tools", "Limiter", "Gate", "Microgate (glitch)", "Ring Modulator", "Frequency Shifter", "Phase Modulation", "Treemonster",
	"Osc: Pulse", "Osc: Pulse (sync)", "Osc: Sawtooth", "Osc: Sine"
};
...
const char mmc_abberations[mmc_num_types][8] = {
	"}x", "1-x","x²","x³",
	"Ÿ",	// custom chars in font for sqrt(x)
	"", // custom chars in font for cubert(x)
	//"exp",	
	"abs","+‚±","±‚+","tri","tr±","pos","neg",">0","<0",">½","<½","pol",
...
struct sample_part
{
	...
	int					MIDIchannel;	// channel part recieves on (defaults to its id but can be stored in multi, but not patches)	
