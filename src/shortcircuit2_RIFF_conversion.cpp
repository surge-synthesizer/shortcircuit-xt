...
int RIFF_FILTER_Type_To_Internal(int Type)
{
	switch(Type)
	{		
	case RFT_None: return ft_none;
	case RFT_SBQ: return ft_biquadSBQ;
	case RFT_Moog: return ft_moogLP4sat;
	case RFT_EQ_2Band: return ft_eq_2band_parametric_A;
	case RFT_EQ_6BandGraphic: return ft_eq_6band;
//	case RFT_MorphEQ: return ft_morpheq;
	case RFT_Limiter: return ft_fx_limiter;
	case RFT_Gate: return ft_fx_gate;
	case RFT_MicroGate: return ft_fx_microgate;
	case RFT_Distortion: return ft_fx_distortion1;
	case RFT_DigiLofi: return ft_fx_bitfucker;
	case RFT_Clipper: return ft_fx_clipper;
	case RFT_Slewer: return ft_fx_slewer;	
	case RFT_RingMod: return ft_fx_ringmod;
	case RFT_FreqShift: return ft_fx_freqshift;
	case RFT_PhaseMod: return ft_fx_phasemod;
	case RFT_Delay: return ft_e_delay;
	case RFT_Reverb: return ft_e_reverb;
	case RFT_Chorus: return ft_e_chorus;
	case RFT_Phaser: return ft_e_phaser;
	case RFT_Rotary: return ft_e_rotary;
	case RFT_FauxStereo: return ft_e_fauxstereo;
	case RFT_FSFlange: return ft_e_fsflange;
	case RFT_FSDelay: return ft_e_fsdelay;
	case RFT_SawOsc: return ft_osc_saw;
	case RFT_SinOsc: return ft_osc_sin;
	case RFT_PulseOsc: return ft_osc_pulse;
	case RFT_PulseOscSync: return ft_osc_pulse_sync;
	case RFT_Treemonster: return ft_fx_treemonster;
	case RFT_Comb1: return ft_comb1;
	case RFT_StereoTools: return ft_fx_stereotools;	
	};
	return ft_none;
}
...
int RIFF_FILTER_Type_From_Internal(int Type)
{
	switch(Type)
	{		
	case ft_none: return RFT_None;
	case ft_biquadSBQ: return RFT_SBQ;
	case ft_moogLP4sat: return RFT_Moog;
	case ft_eq_2band_parametric_A: return RFT_EQ_2Band;
	case ft_eq_6band: return RFT_EQ_6BandGraphic;
//	case ft_morpheq: return RFT_MorphEQ;
	case ft_fx_limiter: return RFT_Limiter;
	case ft_fx_gate: return RFT_Gate;
	case ft_fx_microgate: return RFT_MicroGate;
	case ft_fx_distortion1: return RFT_Distortion;
	case ft_fx_bitfucker: return RFT_DigiLofi;
	case ft_fx_clipper: return RFT_Clipper;
	case ft_fx_slewer: return RFT_Slewer;	
	case ft_fx_ringmod: return RFT_RingMod;
	case ft_fx_freqshift: return RFT_FreqShift;
	case ft_fx_phasemod: return RFT_PhaseMod;
	case ft_e_delay: return RFT_Delay;
	case ft_e_reverb: return RFT_Reverb;
	case ft_e_chorus: return RFT_Chorus;
	case ft_e_phaser: return RFT_Phaser;
	case ft_e_rotary: return RFT_Rotary;
	case ft_e_fauxstereo: return RFT_FauxStereo;
	case ft_e_fsflange: return RFT_FSFlange;
	case ft_e_fsdelay: return RFT_FSDelay;
	case ft_osc_saw: return RFT_SawOsc;
	case ft_osc_sin: return RFT_SinOsc;
	case ft_osc_pulse: return RFT_PulseOsc;
	case ft_osc_pulse_sync: return RFT_PulseOscSync;
	case ft_fx_treemonster: return RFT_Treemonster;
	case ft_comb1: return RFT_Comb1;
	case ft_fx_stereotools: return RFT_StereoTools;	
	};
	return ft_none;
}
...
unsigned char Playmode_To_Internal(unsigned char x)
{
	switch(x)
	{
	case pm_forwardRIFF:					return pm_forward;
	case pm_forward_loopRIFF:				return pm_forward_loop;
	case pm_forward_loop_bidirectionalRIFF:	return pm_forward_loop_bidirectional;
	case pm_forward_loop_until_releaseRIFF:	return pm_forward_loop_until_release;
	case pm_forward_shotRIFF:				return pm_forward_shot;
	case pm_forward_releaseRIFF:			return pm_forward_release;
	case pm_forward_hitpointsRIFF:			return pm_forward_hitpoints;
...
unsigned char Playmode_From_Internal(unsigned char x)
{
	switch(x)
	{
	case pm_forward:					return pm_forwardRIFF;
	case pm_forward_loop:				return pm_forward_loopRIFF;
	case pm_forward_loop_bidirectional:	return pm_forward_loop_bidirectionalRIFF;
	case pm_forward_loop_until_release:	return pm_forward_loop_until_releaseRIFF;
	case pm_forward_shot:				return pm_forward_shotRIFF;
	case pm_forward_release:			return pm_forward_releaseRIFF;
	case pm_forward_hitpoints:			return pm_forward_hitpointsRIFF;
...
{	
	...
	for(int p=0; p<n_filter_iparameters; p++) f->ip[p] = vt_read_int32LE(F->IP[p]);
}
...
void ReadChunkSLFO(RIFF_LFO *E, steplfostruct *e)
{
	...
	for(int j=0; j<32; j++) e->data[j] = vt_read_float32LE(E->StepData[j]);
...
void WriteChunkSLFO(RIFF_LFO *E, steplfostruct *e)
{
	...
	for(int j=0; j<32; j++) E->StepData[j] = vt_write_float32LE(e->data[j]);

	E->Flags = vt_write_int32LE((e->onlyonce ? RLF_OnlyOnce : 0) | (e->cyclemode ? RLF_CycleMode : 0) | (e->temposync ? RLF_TempoSync : 0));
	E->TriggerMode = e->triggermode;
	E->Repeat = e->repeat;
	E->UD[0] = 0;	E->UD[1] = 0;	
}
...
void WriteChunkZonD(RIFF_ZONE *Z, sample_zone *z, int SampleID)
{
	...
	Z->Flags = vt_write_int32LE((z->mute ? RZF_Mute : 0) | (z->ignore_part_polymode ? RZF_IgnorePartPolymode : 0) | (z->reverse ? RZF_Reverse : 0));	
	Z->MuteGroup = z->mute_group;	
	
	Z->N_Hitpoints = z->n_hitpoints;
	
	Z->UD0[0] = 0;	Z->UD0[1] = 0;	Z->UD0[2] = 0;	Z->UD0[3] = 0;	Z->UD0[4] = 0;	
...
void WriteChunkParD(RIFF_PART *P, sample_part *p)
{
	...
	P->Flags = vt_write_int32LE(
		(p->vs_xf_equality?RPF_XFadeEqualPower:0) |
