...
//	Shortcircuit 2
//
//	Copyright 2004-2006 Claes Johanson & Vember Audio
//
//-------------------------------------------------------------------------------------------------------
...
Always traverse the RIFF structure properly, don't make any assumptions about the order or existence of chunks.
Chunks with the same ID/fourCC within a LIST block should be parsed/written in the order of their appearance.
...
RIFF:SC2P
	...
	LIST:ZnLs			// Zone List
		LIST:Zone
			...
			LIST:Fltr
				FltD
	LIST:SmLs			// Sample list. All Child nodes increment the ID, unrecognized or not.
		LIST:WAVE		// Embedded Samples in WAVE format
			data
			fmt 
			..
...
All endpoints of sample-ranges (such as LoopEnd, SampleStop..) are stored EXCLUDING the end-point. Thus the last sample to be played for a zone is (SampleStop - 1).
All undefined (UD) values should be set to 0.
...
// Future versions may have freely assignable modulator types for each slot so the EG vs. SLFO order is important.
...
{		
	...
	unsigned int		SampleID;		// This ID assigns a sample from the LIST:SmLs sample list to the zone  (starting at 0)
