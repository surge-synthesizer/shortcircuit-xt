...
void chorus::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR,
                            float pitch)
{
    setvars(false);
    ...
    1)-vtime)),0,FIRipol_M-1);
                    ...
                    {
                            vo += buffer[(rp-i)&(max_delay_length-1)]*storage->SincTableF321X[sinc +
    FIRipol_N - i - 1];
                    }
    ...
    for (k = 0; k < block_size; k++)
    {
        ...
        for (int j = 0; j < 4; j++)
        {
            ...
            vo = _mm_add_ps(vo, _mm_mul_ps(_mm_load_ps(&SincTableF32[sinc + 4]),
                                           _mm_loadu_ps(&buffer[rp + 4])));
            vo = _mm_add_ps(vo, _mm_mul_ps(_mm_load_ps(&SincTableF32[sinc + 8]),
...
fxdata->p[0].set_type(ct_delaymodtime); fxdata->p[1].set_name("rate");
fxdata->p[1].set_type(ct_lforate); fxdata->p[2].set_name("depth");
fxdata->p[2].set_type(ct_percent); fxdata->p[3].set_name("feedback");
fxdata->p[3].set_type(ct_amplitude); fxdata->p[4].set_name("low cut");
fxdata->p[4].set_type(ct_freq_audible); fxdata->p[5].set_name("high cut");
fxdata->p[5].set_type(ct_freq_audible); fxdata->p[6].set_name("mix");
fxdata->p[6].set_type(ct_percent); fxdata->p[7].set_name("width");
