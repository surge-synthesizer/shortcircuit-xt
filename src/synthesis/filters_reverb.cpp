...
reverb::reverb(float *fp, int *ip) : filter(fp, 0, true, ip)
{
...
void reverb::update_rtime()
{
    ...
    {
        delay_fb[t] =
            powf(db60, delay_time[t] / (256.f * samplerate * powf(2.f, param[rp_decaytime])));
                            ...
                            float pitch)
{
    ...
    if (iparam[0] != shape)
        loadpreset(iparam[0]);
    if ((b == 0) && (fabs(param[rp_roomsize] - lastf[rp_roomsize]) > 0.001f))
        loadpreset(shape);
    if (fabs(param[rp_decaytime] - lastf[rp_decaytime]) > 0.001f)
        update_rtime();
