...
morphEQ::morphEQ(float *fp, void *loader, int *ip) : filter(fp, loader, true, ip)
{
    ...
    parameter_count = 4;
    strcpy(ctrllabel[0], "morph");
    ctrlmode[0] = cm_percent;
    strcpy(ctrllabel[1], "freq offset");
    ctrlmode[1] = cm_mod_freq;
    strcpy(ctrllabel[2], "gain offset");
    ctrlmode[2] = cm_mod_decibel;
    strcpy(ctrllabel[3], "BW offset");
    ctrlmode[3] = cm_mod_percent;
    ...
    if (loader && fp)
    {
        ...
        int src = max(ip[0], 0);
        int dst = max(ip[1], 0);
        if (src >= mload->n_loaded)
            src = 0;
        if (dst >= mload->n_loaded)
            dst = 0;
    ...
    gaintarget = 1.f;
    lastparam[0] = -165464686; // se till att cal_coeffs altlid utf�rs f�rsta g�ngen
}
...
int morphEQ::get_ip_count() { return 2; }

const char *morphEQ::get_ip_label(int ip_id) { return (ip_id == 1) ? "snapshot 2" : "snapshot 1"; }
...
const char *morphEQ::get_ip_entry_label(int ip_id, int c_id)
{
    if ((ip_id & 0xfe) != 0)
        return 0;
    if (c_id > this->get_ip_entry_count(ip_id))
        return 0;
...
void morphEQ::calc_coeffs()
{
        ...
        (lastparam[3] != param[3]))
    {
        ...
        if (b_active[0])
            b[0].coeff_HP(biquadunit::calc_omega(snap[0].bands[0].freq * morph_m1 +
                                                 snap[1].bands[0].freq * morph + param[1]),
                          Q);
        if (b_active[7])
            b[7].coeff_LP2B(biquadunit::calc_omega(snap[0].bands[7].freq * morph_m1 +
                                                   snap[1].bands[7].freq * morph + param[1]),
                            Q);
        ...
        for (int i = 1; i < 7; i++)
        {
            if (b_active[i])
                b[i].coeff_peakEQ(
                    biquadunit::calc_omega(snap[0].bands[i].freq * morph_m1 +
                                           snap[1].bands[i].freq * morph + param[1]),
...
float morphEQ::get_freq_graph(float f)
{
    ...
    for (int k = 0; k < 8; k++)
        if (b_active[k])
            y *= b[k].plot_magnitude(f);

    return y * gain.get_target();
}

void morphEQ::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR,
                             float pitch)
{
    calc_coeffs();
    ...
    if (b_active[1])
        b[1].process_block_to(dataoutL, dataoutR, dataoutL, dataoutR);
    if (b_active[2])
        b[2].process_block_to(dataoutL, dataoutR, dataoutL, dataoutR);
    if (b_active[3])
        b[3].process_block_to(dataoutL, dataoutR, dataoutL, dataoutR);
    if (b_active[4])
        b[4].process_block_to(dataoutL, dataoutR, dataoutL, dataoutR);
    if (b_active[5])
        b[5].process_block_to(dataoutL, dataoutR, dataoutL, dataoutR);
    if (b_active[6])
        b[6].process_block_to(dataoutL, dataoutR, dataoutL, dataoutR);
    if (b_active[7])
        b[7].process_block_to(dataoutL, dataoutR, dataoutL, dataoutR);
}
...
void morphEQ::process(float *datain, float *dataout, float pitch)
{
    ...
    if (b_active[1])
        b[1].process_block(dataout);
    if (b_active[2])
        b[2].process_block(dataout);
    if (b_active[3])
        b[3].process_block(dataout);
    if (b_active[4])
        b[4].process_block(dataout);
    if (b_active[5])
        b[5].process_block(dataout);
    if (b_active[6])
        b[6].process_block(dataout);
    if (b_active[7])
        b[7].process_block(dataout);
}
...
EQ2BP_A::EQ2BP_A(float *fp, int *ip) : filter(fp, 0, true, ip)
{
    ...
    parameter_count = 6;
    strcpy(ctrllabel[0], "gain");
    ctrlmode[0] = cm_mod_decibel;
    strcpy(ctrllabel[1], "freq");
    ctrlmode[1] = cm_frequency_audible;
    strcpy(ctrllabel[2], "BW");
    ctrlmode[2] = cm_eq_bandwidth;
    strcpy(ctrllabel[3], "gain");
    ctrlmode[3] = cm_mod_decibel;
    strcpy(ctrllabel[4], "freq");
    ctrlmode[4] = cm_frequency_audible;
    strcpy(ctrllabel[5], "BW");
    ctrlmode[5] = cm_eq_bandwidth;
    ...
    lastparam[0] = -1654646816; // se till att cal_coeffs altlid utf�rs f�rsta g�ngen
}
...
void EQ2BP_A::calc_coeffs()
{
    ...
    {
        parametric[0].coeff_orfanidisEQ(biquadunit::calc_omega(param[1]), param[2],
                                        dB_to_linear(param[0]), calc_GB_type_B(iparam[0], param[0]),
                                        1);
        parametric[1].coeff_orfanidisEQ(biquadunit::calc_omega(param[4]), param[5],
                                        dB_to_linear(param[3]), calc_GB_type_B(iparam[1], param[3]),
                                        1);
        memcpy(lastparam, param, sizeof(lastparam));
    }
}
...
void EQ2BP_A::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR,
                             float pitch)
{
    calc_coeffs();
...
EQ2BP_B::EQ2BP_B(float *fp) : filter(fp)
{
        ...
        strcpy(ctrllabel[2], "BW");					ctrlmode[2] =
cm_eq_bandwidth; strcpy(ctrllabel[3], "gain");				ctrlmode[3] =
cm_mod_decibel; strcpy(ctrllabel[4], "freq");				ctrlmode[4] =
cm_frequency_audible;
        strcpy(ctrllabel[5], "BW");					ctrlmode[5] =
cm_eq_bandwidth;
...
{
        assert(param);
        if((lastparam[0] != param[0])||(lastparam[1] != param[1])||(lastparam[2] !=
param[2])||(lastparam[3] != param[3])||(lastparam[4] != param[4])||(lastparam[5] != param[5]))
        {
                parametric[0].coeff_orfanidisEQ(biquadunit::calc_omega(param[1]),param[2],dB_to_linear(param[0]),
calc_GB_type_B(param[0]),1);
                parametric[1].coeff_orfanidisEQ(biquadunit::calc_omega(param[4]),param[5],dB_to_linear(param[3]),
calc_GB_type_B(param[3]),1); memcpy(lastparam,param,sizeof(lastparam));
        }
}
...
EQ6B::EQ6B(float *fp) : filter(fp)
{
    ...
    parameter_count = 6;
    strcpy(ctrllabel[0], "100");
    ctrlmode[0] = cm_mod_decibel;
    strcpy(ctrllabel[1], "250");
    ctrlmode[1] = cm_mod_decibel;
    strcpy(ctrllabel[2], "630");
    ctrlmode[2] = cm_mod_decibel;
    strcpy(ctrllabel[3], "1.6k");
    ctrlmode[3] = cm_mod_decibel;
    strcpy(ctrllabel[4], "5k");
    ctrlmode[4] = cm_mod_decibel;
    strcpy(ctrllabel[5], "12k");
    ctrlmode[5] = cm_mod_decibel;
...
void EQ6B::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR,
                          float pitch)
{
    calc_coeffs();
    parametric[0].process_block_to(datainL, datainR, dataoutL, dataoutR);
...
{
    return parametric[0].plot_magnitude(f) * parametric[1].plot_magnitude(f) *
           parametric[2].plot_magnitude(f) * parametric[3].plot_magnitude(f) *
           parametric[4].plot_magnitude(f) * parametric[5].plot_magnitude(f);
}
...
float LP2HP2_morph::get_freq_graph(float f) { return this->f.plot_magnitude(f); }
