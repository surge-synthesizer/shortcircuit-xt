...
const char	str_freqdef[] = ("f,-5,0.04,6,5,Hz"),
				str_freqmoddef[] = ("f,-12,0.04,12,0,oct"),
				str_timedef[] = ("f,-10,0.1,10,4,s"),
				str_lfofreqdef[] = ("f,-5,0.04,6,4,Hz"),
				str_percentdef[] = ("f,0,0.005,1,1,%"),
				str_percentbpdef[] = ("f,-1,0.005,1,1,%"),
				str_percentmoddef[] = ("f,-32,0.005,32,1,%"),
				str_dbdef[] = ("f,-96,0.1,12,0,dB"),
				str_dbbpdef[] = ("f,-48,0.1,48,0,dB"),
				str_dbmoddef[] = ("f,-96,0.1,96,0,dB"),			
				str_mpitch[] = ("f,-96,0.04,96,0,"),
				str_bwdef[] = ("f,0.001,0.005,6,0,oct");	
...
public:
	...
	void calc_coeffs();
	virtual bool init_freq_graph(){ calc_coeffs(); return true; }	
...
public:
	...
	void process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch);
	virtual void init_params();	
...
public:
	...
	void process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch);
	virtual void init_params();
	virtual void suspend(){ bq[0].suspend(); bq[1].suspend(); }
	void calc_coeffs();
	virtual bool init_freq_graph(){ calc_coeffs(); return true; }	
	virtual float get_freq_graph(float f);
protected:	
...
public:
	...
	void process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch);
	virtual void init_params();
	virtual void suspend(){ parametric[0].suspend(); parametric[1].suspend(); }
	void calc_coeffs();
	virtual bool init_freq_graph(){ calc_coeffs(); return true; }	
	virtual float get_freq_graph(float f);
	virtual int		get_ip_count();
...
public:
	...
	virtual bool init_freq_graph(){ calc_coeffs(); return true; }	// filter z-plot honk (visa p� waveformdisplay)
	virtual float get_freq_graph(float f);
};
...
public:
	...
	virtual void suspend(){ b[0].suspend(); b[1].suspend(); b[2].suspend(); b[3].suspend(); b[4].suspend(); b[5].suspend(); b[6].suspend(); b[7].suspend(); }
protected:	
	bool b_active[8];
	float gaintarget;
	meq_snapshot snap[2];	
...
public:
	...
	void process(float *data, float pitch);
	virtual void init_params();
	virtual void suspend(){ f.suspend(); feedback = 0; }
	virtual int tail_length(){ return tail_infinite; }
protected:
	biquadunit f;
	double feedback;
...
public:
	...
	virtual void init_params();
	virtual void suspend(){ bq[0].suspend(); bq[1].suspend(); v[0]=0; v[1	]=0; }
	void calc_coeffs();
	virtual bool init_freq_graph(){ calc_coeffs(); return true; }	
	virtual float get_freq_graph(float f);
protected:
