...
fauxstereo::fauxstereo(float *ep, int *ip) : filter(ep)
{
	...
	parameter_count = 3;
	strcpy(ctrllabel[0], "amplitude");		ctrlmode[0] = cm_decibel;
	strcpy(ctrllabel[1], "delay");			ctrlmode[1] = cm_time1s;
	strcpy(ctrllabel[2], "source (M/S)");	ctrlmode[2] = cm_percent;	
...
void fauxstereo::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{		
	fp[0] = param[1];
	fp[1] = 0;
...
fs_flange::fs_flange(float *ep, int*pi) : filter(ep)
{
	...
	strcpy(ctrllabel[0], "shift");			ctrlmode[0] = cm_frequency_hz_bi;
	strcpy(ctrllabel[1], "rel. shift R");	ctrlmode[1] = cm_mod_percent;
	strcpy(ctrllabel[2], "feedback");		ctrlmode[2] = cm_decibel;
...
void fs_flange::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{		
	...
	{					
		fs_buf[0][k] = (float)tanh_fast(datainL[k] + feedback.v*fs_buf[0][k]);	// denormal honk i fs_buf?
...
freqshiftdelay::freqshiftdelay(float *ep, int *ip) : filter(ep)
{
	...
	parameter_count = 3;	
	strcpy(ctrllabel[0], "time");			ctrlmode[0] = cm_time1s;
	strcpy(ctrllabel[1], "feedback");		ctrlmode[1] = cm_decibel;
	strcpy(ctrllabel[2], "freq shift");		ctrlmode[2] = cm_frequency_khz_bi;	
...
void freqshiftdelay::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{	
	f_fs[0] = param[2];
	f_fs[1] = 1.f;
