...
float LP2A::get_freq_graph(float f)
{
	return lp.plot_magnitude(f);
}
...
void LP2B::calc_coeffs()
{
	...
	if((lastparam[0] != param[0])||(lastparam[1] != param[1]))
	{		
		lp.coeff_LP2B(biquadunit::calc_omega(param[0]),M_SQRT1_2/(1-limit_range(param[1],0.f,0.999f)));
		memcpy(lastparam,param,sizeof(lastparam));
	}	
}
float LP2B::get_freq_graph(float f)
{
	return lp.plot_magnitude(f);
}
...
void LP2B::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	calc_coeffs();
...
{			
	...
	{		
		...
		for(int i=1; i<n; i++) q *= param[1];

		switch(iparam[0])
		{
		case sb_LP:
			bq[0].coeff_LP2B(biquadunit::calc_omega(param[0]),M_SQRT1_2/(1-limit_range(q,0.f,0.999f)));
			break;
		case sb_HP:
			bq[0].coeff_HP(biquadunit::calc_omega(param[0]),M_SQRT1_2/(1-limit_range(q,0.f,0.999f)));
			break;
		case sb_BP:
			bq[0].coeff_BP2AQ(biquadunit::calc_omega(param[0]),M_SQRT1_2/(1-limit_range(q,0.f,0.999f)));
			//bq[0].coeff_orfanidisEQ(biquadunit::calc_omega(param[0]),1/(1-limit_range(q,0.f,0.999f)),1, (1/sqrt(2.0)),0);
			break;
		case sb_Notch:
			bq[0].coeff_NOTCHQ(biquadunit::calc_omega(param[0]),M_SQRT1_2/(1-limit_range(q,0.f,0.999f)));
			break;
		}
...
float superbiquad::get_freq_graph(float f)
{
	...
	int n = min(4,(iparam[1]+1));
	for(int i=1; i<n; i++) r *= bq[i].plot_magnitude(f);
	return r;
}
...
void superbiquad::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	calc_coeffs();
...
float HP2A::get_freq_graph(float f)
{
	return hp.plot_magnitude(f);
}
...
void HP2A::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	calc_coeffs();
...
BP2A::BP2A(float *fp) : filter(fp)
{
	...
	strcpy(ctrllabel[1], ("bandwidth"));			ctrlmode[1] = cm_percent;		
	strcpy(ctrlmode_desc[0], str_freqdef);
...
{		
	if((lastparam[0] != param[0])||(lastparam[1] != param[1]))
	{		
		bq.coeff_orfanidisEQ(biquadunit::calc_omega(param[0]),1/(1 / (0.02 + 30*param[1]*param[1])),1,M_SQRT1_2,0);
		memcpy(lastparam,param,sizeof(lastparam));
	}	
}
float BP2A::get_freq_graph(float f)
{
...
BP2B::BP2B(float *fp) : filter(fp)
{
	...
	parameter_count = 2;
	strcpy(ctrllabel[0], ("frequency"));			ctrlmode[0] = cm_frequency_audible;
	strcpy(ctrllabel[1], ("bandwidth"));			ctrlmode[1] = cm_eq_bandwidth;		
	strcpy(ctrlmode_desc[0], str_freqdef);
	strcpy(ctrlmode_desc[1], str_bwdef);
...
float BP2B::get_freq_graph(float f)
{
	return bq.plot_magnitude(f);
}
...
void BP2B::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	calc_coeffs();
...
BP2AD::BP2AD(float *fp) : filter(fp)
{
	...
	strcpy(ctrllabel[0], ("frequency"));					ctrlmode[0] = cm_frequency_audible;
	strcpy(ctrllabel[1], ("bandwidth"));					ctrlmode[1] = cm_percent;
	strcpy(ctrllabel[2], ("abs. offset"));				ctrlmode[2] = cm_frequency0_2k;
	strcpy(ctrllabel[3], ("rel. offset"));				ctrlmode[3] = cm_mod_freq;
	lastparam[0] = -1654646816;
...
{	
	...
	if((lastparam[0] != param[0])||(lastparam[1] != param[1])||(lastparam[2] != param[2])||(lastparam[3] != param[3]))
	{
		...
		bq[0].coeff_orfanidisEQ(biquadunit::calc_omega(freq0),1/(1 / (0.02 + 30*param[1]*param[1])),1,M_SQRT1_2,0);
		bq[1].coeff_orfanidisEQ(biquadunit::calc_omega(freq1),1/(1 / (0.02 + 30*param[1]*param[1])),1,M_SQRT1_2,0);
		memcpy(lastparam,param,sizeof(lastparam));
	}
}

float BP2AD::get_freq_graph(float f)
{
	return bq[0].plot_magnitude(f)+bq[1].plot_magnitude(f);
}

void BP2AD::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	calc_coeffs();
...
float PKA::get_freq_graph(float f)
{
	return bq.plot_magnitude(f);
}
...
void PKA::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	calc_coeffs();
	bq.process_block_to(datainL,datainR,dataoutL,dataoutR);
}
...
float PKAD::get_freq_graph(float f)
{
	return bq[0].plot_magnitude(f)+bq[1].plot_magnitude(f);
}

void PKAD::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	calc_coeffs();
	Align16 float d[2][block_size];	
	bq[0].process_block_to(datainL,datainR,d[0],d[1]);
...
void NOTCH::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	calc_coeffs();
	bq.process_block_to(datainL,datainR,dataoutL,dataoutR);
}
...
LPHP_ser::LPHP_ser(float *fp) : filter(fp)
{
	...
	parameter_count = 4;
	strcpy(ctrllabel[0], ("HP freq"));				ctrlmode[0] = cm_frequency_audible;
	strcpy(ctrllabel[1], ("HP reso"));				ctrlmode[1] = cm_percent;
	strcpy(ctrllabel[2], ("LP freq"));				ctrlmode[2] = cm_frequency_audible;
	strcpy(ctrllabel[3], ("LP reso"));				ctrlmode[3] = cm_percent;		
	strcpy(ctrlmode_desc[0], str_freqdef);
	strcpy(ctrlmode_desc[1], str_percentdef);
...
{	
	bq[0].coeff_HP(biquadunit::calc_omega(param[0]),M_SQRT1_2/(1-limit_range(param[1],0.f,0.999f)));
	bq[1].coeff_LP2B(biquadunit::calc_omega(param[2]),M_SQRT1_2/(1-limit_range(param[3],0.f,0.999f)));
}
...
void LPHP_ser::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	calc_coeffs();
	bq[0].process_block_to(datainL,datainR,dataoutL,dataoutR);
...
LPHP_par::LPHP_par(float *fp) : filter(fp)
{
	...
	parameter_count = 4;
	strcpy(ctrllabel[0], ("LP freq"));				ctrlmode[0] = cm_frequency_audible;
	strcpy(ctrllabel[1], ("LP reso"));				ctrlmode[1] = cm_percent;
	strcpy(ctrllabel[2], ("HP freq"));				ctrlmode[2] = cm_frequency_audible;
	strcpy(ctrllabel[3], ("HP reso"));				ctrlmode[3] = cm_percent;	
	strcpy(ctrlmode_desc[0], str_freqdef);
	strcpy(ctrlmode_desc[1], str_percentdef);
	strcpy(ctrlmode_desc[2], str_freqdef);
	strcpy(ctrlmode_desc[3], str_percentdef);
...
{
	bq[0].coeff_LP(biquadunit::calc_omega(param[0]),M_SQRT1_2/(1-limit_range(param[1],0.f,0.999f)));
	bq[1].coeff_HP(biquadunit::calc_omega(param[2]),M_SQRT1_2/(1-limit_range(param[3],0.f,0.999f)));
}
...
void LPHP_par::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	calc_coeffs();
	Align16 float d[2][block_size];	
	bq[0].process_block_to(datainL,datainR,d[0],d[1]);
