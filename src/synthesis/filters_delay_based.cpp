...
{	
	...
	strcpy(ctrllabel[1], "feedback");			ctrlmode[1] = cm_percent_bipolar;		
	wpos = 0;
...
void COMB1::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{	
	...
	{			
		...
		{
			outputL += delayloop[0][(wpos-i_dt-i)&(comb_max_delay-1)]*SincTableF32[sinc + FIRipol_N - i];			
			outputR += delayloop[1][(wpos-i_dt-i)&(comb_max_delay-1)]*SincTableF32[sinc + FIRipol_N - i];			
		}		
...
{	
	...
	{	
		...
		for(int i=0; i<FIRipol_N; i++)
		{
			output += delayloop[0][(wpos-i_dt-i)&(comb_max_delay-1)]*SincTableF32[sinc + FIRipol_N - i];			
		}
...
{	
	...
	parameter_count = 2;
	strcpy(ctrllabel[0], "delay time");			ctrlmode[0] = cm_frequency_audible;
	strcpy(ctrllabel[1], "feedback");			ctrlmode[1] = cm_percent_bipolar;		
	wpos = 0;
...
{	
	...
	{	
		...
		for(int i=0; i<FIRipol_N; i++)
		{
			output += delayloop[(wpos-i_dt-i)&(comb_max_delay-1)]*SincTableF32[sinc + FIRipol_N - i];			
		}
