...
{	
	...
	parameter_count = 5;
	strcpy(ctrllabel[0], ("samplerate"));			ctrlmode[0] = cm_frequency_samplerate;
	strcpy(ctrllabel[1], ("bitdepth"));			ctrlmode[1] = cm_bitdepth_16;
	strcpy(ctrllabel[2], ("zeropoint"));			ctrlmode[2] = cm_percent;
	strcpy(ctrllabel[3], ("cutoff"));				ctrlmode[3] = cm_frequency_audible;
	strcpy(ctrllabel[4], ("resonance"));			ctrlmode[4] = cm_percent;
	for(int c=0; c<2; c++)
	{
		time[c] = 0;
		postslew[c] = 0;
	...
	lp = (LP2B*) _mm_malloc(sizeof(LP2B),16);
...
{	
	...
	parameter_count = 5;
	strcpy(ctrllabel[0], ("drive"));				ctrlmode[0] = cm_percent;
	strcpy(ctrllabel[1], ("EQ freq"));			ctrlmode[1] = cm_frequency_audible;
	strcpy(ctrllabel[2], ("EQ gain"));			ctrlmode[2] = cm_decibel;
	// eq Q
	strcpy(ctrllabel[3], ("Post cutoff"));		ctrlmode[3] = cm_frequency_audible;
	strcpy(ctrllabel[4], ("Post reso"));			ctrlmode[4] = cm_percent;
	time = 0;
	postslew = 0;
	level = 0;
...
{	
	...
	parameter_count = 4;
	strcpy(ctrllabel[0], ("threshold"));			ctrlmode[0] = cm_mod_decibel;
	strcpy(ctrllabel[1], ("pitch"));				ctrlmode[1] = cm_mod_pitch;	
	strcpy(ctrllabel[2], ("mode blend"));			ctrlmode[2] = cm_percent;	
	strcpy(ctrllabel[3], ("lo/hi cut"));			ctrlmode[3] = cm_frequency_audible;	
...
void treemonster::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{	
	...
	if (iparam[0]) locut.coeff_LP2B(biquadunit::calc_omega(param[3]),0.707);
	else locut.coeff_HP(biquadunit::calc_omega(param[3]),0.707);
	...
	gain[0].fade_2_blocks_to(tbuf[0],dataoutL,tbuf[1],dataoutR,dataoutL,dataoutR,block_size_quad);
...
{			
	...
	{		
		...
		{
			if(tbuf[k] > db_to_linear(param[0])) osc[0].set_rate((M_PI / max(2.f,length[0])) * powf(2.0,param[1] * (1/12.f)));
			length[0] = 0.f;
		}
		osc[0].process();
		dataout[k] = osc[0].r;
...
{ 
	if(ip_id == 0) return ("filter");
	return (""); 
}

int treemonster::get_ip_entry_count(int ip_id)
{
	return 2;
}
...
{	
	...
	parameter_count = 2;
	strcpy(ctrllabel[0], ("drive"));				ctrlmode[0] = cm_mod_decibel;
	strcpy(ctrllabel[1], ("threshold"));			ctrlmode[1] = cm_mod_decibel;	
...
void clipper::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{		
...
void fdistortion::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{		
...
{	
	...
	strcpy(ctrllabel[1], ("loop"));				ctrlmode[1] = cm_percent;	
	strcpy(ctrllabel[2], ("threshold"));			ctrlmode[2] = cm_decibel;
	strcpy(ctrllabel[3], ("reduction"));			ctrlmode[3] = cm_decibel;
...
void microgate::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	...
	{	
		...
		if (gate_zc_sync[0]) 		
		{
			...
			{				
				...
				bufpos[0]++;
				if (bufpos[0]>=buflength[0]) bufpos[0] = 0;
			}
		}
		else
		...
		if (gate_zc_sync[1]) 		
		{
			...
			{				
				...
				bufpos[1]++;
				if (bufpos[1]>=buflength[1]) bufpos[1] = 0;
			}
		}
		else
...
{	
	...
	parameter_count = 6;
	strcpy(ctrllabel[0], ("pre:gain"));		ctrlmode[0] = cm_mod_decibel;	
	strcpy(ctrllabel[1], ("pre:freq"));		ctrlmode[1] = cm_frequency_audible;
	strcpy(ctrllabel[2], ("pre:BW"));			ctrlmode[2] = cm_eq_bandwidth;	
	strcpy(ctrllabel[3], ("slew rate"));		ctrlmode[3] = cm_frequency_audible;		
	strcpy(ctrllabel[4], ("post:gain"));		ctrlmode[4] = cm_mod_decibel;	
	strcpy(ctrllabel[5], ("post:freq"));		ctrlmode[5] = cm_frequency_audible;	
...
void fslewer::calc_coeffs()
{
	...
	if((lastparam[0] != param[0])||(lastparam[1] != param[1])||(lastparam[2] != param[2])||(lastparam[4] != param[4])||(lastparam[5] != param[5]))
	{
		bq[0].coeff_orfanidisEQ(biquadunit::calc_omega(param[1]),param[2],dB_to_linear(param[0]), dB_to_linear(param[0]*0.5),1);		
		bq[1].coeff_orfanidisEQ(biquadunit::calc_omega(param[5]),2,dB_to_linear(param[4]), dB_to_linear(param[4]*0.5),1);		
		memcpy(lastparam,param,sizeof(lastparam));
	}	
}

float fslewer::get_freq_graph(float f)
{
...
void fslewer::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	assert(param);	
	calc_coeffs();		
