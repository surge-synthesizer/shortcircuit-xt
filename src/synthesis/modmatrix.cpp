...
int modmatrix::get_destination_value_int(int id)
{ 
	return Float2Int(fdst[id]); 
}
...
{		
	...
	if(zone)	// only available for the zone
	{
		...
		add_source(RMS_Filter1ModOut, "f1modout",	voice ? &voice->filter_modout[0] : 0,"Filter 1 Mod Out");
		add_source(RMS_Filter2ModOut, "f2modout",	voice ? &voice->filter_modout[1] : 0,"Filter 2 Mod Out");		
	}
	...
	add_source(RMS_ChAftertouch, "channelAT",		control ? &control[c_channel_aftertouch] : 0,"Ch Aftertouch");
	...
	if(zone)
	{
		...
		add_destination(RMD_BalanceAux1, md_aux_balance,"aux_balance",cm_mod_percent,"aux balance");
		add_destination(RMD_AmplitudeAux2, md_aux2_level,"aux2_level",cm_mod_decibel,"aux2 level");
		add_destination(RMD_BalanceAux2, md_aux2_balance,"aux2_balance",cm_mod_percent,"aux2 balance");
		...
		{
			filter* tempf=0; 
			if (zone) tempf = spawn_filter(zone->Filter[fs].type,zone->Filter[fs].p,zone->Filter[fs].ip,0,false);			
			...
			for(unsigned int fp=0; fp<6; fp++)
			{
				...
				if (tempf) ct = tempf->get_parameter_ctrlmode(fp);
				
				ct = get_mod_mode(ct);				

				add_destination(RMD_Filter1Param0 + 0x10*fs + fp, md_filter1prm0+fs*(md_filter2prm0-md_filter1prm0)+fp,idname,ct,txt);	
			}
			spawn_filter_release(tempf);
		}

		add_destination(RMD_SampleStart, md_sample_start,"samplestart",cm_mod_percent,"zone start");
		add_destination(RMD_LoopStart, md_loop_start,"loopstart",cm_mod_percent,"loop start");		
		add_destination(RMD_LoopLength, md_loop_length,"looplength",cm_mod_percent,"loop length");
	...
	else
	{
		...
		add_destination(RMD_PartPreFilterGain, md_part_prefilter_gain,"pfg",cm_mod_decibel,"p.f. gain");
		add_destination(RMD_PartBalance, md_part_pan,"balance",cm_mod_percent);
		add_destination(RMD_PartAmplitudeAux1, md_part_aux_level,"aux_level",cm_mod_decibel,"aux level");
		add_destination(RMD_PartBalanceAux1, md_part_aux_balance,"aux_balance",cm_mod_percent,"aux balance");
		add_destination(RMD_PartAmplitudeAux2, md_part_aux2_level,"aux2_level",cm_mod_decibel,"aux2 level");
		add_destination(RMD_PartBalanceAux2, md_part_aux2_balance,"aux2_balance",cm_mod_percent,"aux2 balance");

		add_destination(RMD_PartFilter1Mix, md_part_filter1mix,"f1mix",cm_mod_percent,"filter 1 mix");
		add_destination(RMD_PartFilter2Mix, md_part_filter2mix,"f2mix",cm_mod_percent,"filter 2 mix");		

		for(unsigned int fs=0; fs<2; fs++)
		{
			filter* tempf=0; 
			if (part) tempf = spawn_filter(part->Filter[fs].type,part->Filter[fs].p,part->Filter[fs].ip,0,false);			
			...
			for(unsigned int fp=0; fp<n_filter_parameters; fp++)
			{
				...
				add_destination(RMD_PartFilter1Param0 + 0x10*fs + fp, md_part_filter1prm0+fs*(md_part_filter2prm0-md_part_filter1prm0)+fp,idname,ct,txt);	
			}
			spawn_filter_release(tempf);
		}
	}	
...
{
	if((source<0)||(source>num_switchable_sources)) return 0;
	if(!zone) return 0;	
...
void modmatrix::add_destination(unsigned char RIFFID, int id, const char* name, int control_type, const char* dispname)
{		
...
void modmatrix::add_source(unsigned char RIFFID, const char* name, float *fptr, const char* dispname)
{
	mm_src t;
...
{
	//return (int)dst.size();
	if(zone) return md_num_zone_destinations;
	else return md_num_part_destinations;
}
...
bool modmatrix::check_NC(sample_zone *z)
{
	...
	for(int i=0; i<nc_entries; i++)
	{
		...
		{			
			...
			if((val < z->nc[i].low)||(val > z->nc[i].high)) return false;
		}
	}
	...
	for(int i=(layer*num_layer_ncs); i<(layer*num_layer_ncs + num_layer_ncs); i++)
	{
		if(src[part->nc[i].source].fptr)
		{
			...
			if((val < part->nc[i].low)||(val > part->nc[i].high)) return false;
		}
	}
...
//	pb_down = max(0,-control[c_pitch_bend]);
	...
	{			
		if(part->mm[i].destination && (part->mm[i].source || part->mm[i].source2) && (part->mm[i].active)&&(src[part->mm[i].source].fptr))
		{
			...
			if(src[part->mm[i].source2].fptr) tmodulation *= *src[part->mm[i].source2].fptr;

			if(curve) tmodulation = do_curve(curve,tmodulation);
...
{	
	...
	/*for(i=0; i<mm_entries; i++)
	{	
		if((zone->mm[i].destination >= md_MM0_depth)&&(zone->mm[i].active)&&(src[zone->mm[i].source].fptr))
		{
	...
	for(i=0; i<mm_entries; i++) 
	{			
		if(zone->mm[i].destination && (zone->mm[i].source || zone->mm[i].source2) &&(zone->mm[i].active)&&(src[zone->mm[i].source].fptr))
		{
			int dest = zone->mm[i].destination;
			int curve = zone->mm[i].curve;
			float strength = zone->mm[i].strength; // + fdst[md_MM0_depth+i];
			float tmodulation = *src[zone->mm[i].source].fptr;
			if(src[zone->mm[i].source2].fptr) tmodulation *= *src[zone->mm[i].source2].fptr;
...
int get_mm_source_id(const char *txt)
{
	...
	for(k=0; k<n; k++)
	{
		if(!strcmp(txt,mm.get_source_idname(k))) return k;
	}
	return 0;
}
...
int get_mm_dest_id(const char *txt)
{
	...
	for(k=0; k<n; k++)
	{
		if(!strcmp(txt,mm.get_destination_idname(k))) return k;
	}
	return 0;
}
...
int modmatrix::SourceRiffIDToInternal(unsigned char ID)
{
	...
	for(k=0; k<n; k++)
	{
		if(ID == get_source_RIFFID(k)) return k;
	}
	return 0;
}
...
int modmatrix::DestinationRiffIDToInternal(unsigned char ID)
{
	...
	for(k=0; k<n; k++)
	{
		if(ID == get_destination_RIFFID(k)) return k;
	}
	return 0;
}
