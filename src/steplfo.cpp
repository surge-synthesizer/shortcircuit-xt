	...
	case lp_noise_mean5:
		{
			...
			float noisev[32];
			int nmean=1,j;
			if (id == lp_noise_mean3) nmean = 3;
			if (id == lp_noise_mean5) nmean = 5;
...
void steplfo::assign(steplfostruct *settings, float *rate, timedata *td)
{
	...
	state = (state + 1) % settings->repeat;	// move state one step ahead to reflect the lag in the interpolation
	for(int i=0; i<4; i++) wf_history[i] = settings->data[((state + settings->repeat - i) % settings->repeat) & 0x1f];
...
/*void steplfo::process(int samples){	
	if(settings->syncmode)
	{
		...
		{
			phase += min(32,samples * syncratio[settings->syncmode] * td->tempo/60 * samplerate_inv);
			while (phase > 1.0f)
			{
				state_tminus1 = state;
				state++;
