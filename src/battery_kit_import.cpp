	...
	while(slot)
	{
		...
		while(samplefile)
		{
			...
			if (add_zone(fn,&newzone,channel))
			{
				...
				zones[newzone].AEG.sustain = powf((float)dval / 127,1/3);	// battery is 3rd order -> 2nd order
				...
				{	// AHDSR mode
					// do nothing
					if(status & 64) zones[newzone].playmode = pm_forward_loop;
				}else
				{	// AHD mode
				...
				if(status & 8) zones[newzone].keytrack = 1;
				else zones[newzone].keytrack = 0;
