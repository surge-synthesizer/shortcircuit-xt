...
//	this->effect = effect;
	...
	for(c=0; c<n_sampler_parts; c++)
	{
		...
		partv[c].mm->assign(conf,0,&parts[c],0,&controllers[n_controllers*c],automation,&time_data);	
	}
...
{		
	...
	if (dbSampleListDataPtr) free(dbSampleListDataPtr);	
}

bool sampler::zone_exist(int id)
{
	if(id >= max_zones) return false;
...
{ 
	for (int p=0; p<n_sampler_parts; p++)
	{
		if (parts[p].MIDIchannel == channel)
		{
			controllers_target[p*n_controllers + c_channel_aftertouch] = (float)value * (1.f/127.0f); 
		}
	}
}
...
{
	if(!zone_exists[z]) return;
	
	modmatrix mm;
	mm.assign(conf,&zones[z],0);
	zones[z].element_active = 
		(mm.is_source_used(ss_LFO1) & ve_LFO1) |
		(mm.is_source_used(ss_LFO2) & ve_LFO2) |
		(mm.is_source_used(ss_LFO3) & ve_LFO3) |
		(mm.is_source_used(ss_EG2) & ve_EG2);
}
...
bool sampler::get_key_name(char *str, int channel, int key)
{
	for(int z=0; z<max_zones; z++)
	{
		if(zone_exists[z])
		{
			...
			if ((zones[z].key_low <= zkey) && (zones[z].key_high >= zkey) && (parts[p].MIDIchannel == channel))
			{
...
{	
	...
	// flytta loop & slices-transfer fr�n samplingen till en egen funktion n�r det �r uppdateringar av det s� det bara finns p� ett st�lle
		...
		if(samples[s]->meta.playmode_present) zones[i].playmode = samples[s]->meta.playmode;
...
bool sampler::replace_zone(int z, const char *filename)
{
	...
	//if (samples[s]->smpl_chunk.dwMIDIUnityNote) zones[z].key_root = (char) samples[s]->smpl_chunk.dwMIDIUnityNote;
	if(samples[s]->meta.loop_present)
	{
...
//		case pm_forward_loop_crossfade:
	...
	{
		switch(zones[z].playmode)		// if loop-points aren't present, but the playmode want to use it, change to pm_forward
		{			
		case pm_forward_loop:
		case pm_forward_loop_bidirectional:
		//case pm_forward_loop_crossfade:
...
void sampler::part_init(int p, bool clear_zones, bool leave_outputs)
{
	...
	} else memset(&parts[p],0,sizeof(sample_part));	
	...
	if (clear_zones) part_clear_zones(p);
}
...
void sampler::Preview::Start(const wchar_t *Filename)
{
	...
	{		
		...
		mpVoice->play(mpSample, &mZone, &mPart, 60, 127, 0, mpParent->controllers, mpParent->automation, 1.f );		
