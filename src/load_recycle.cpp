...
//			float num_samples = (float)info.fPPQLength * ppqratio;			
			...
			float num_samples = ppqratio * (float)sliceInfo.fPPQPos + (float)sliceInfo.fSampleLength;
			...
			// allocate memory;
			uint32 samplesizewithmargin = this->sample_length+2*FIRipol_N + block_size + FIRoffset;
			...
			if(channels==2)
			{
				...
				for(i=0; i<samplesizewithmargin; i++) sample_data_R[i] = 0.0f;
			} else sample_data_R = 0;
...
bailout:
	...
	if (sample_data_R) delete sample_data_R;
	REX::REXUnloadDLL();
	return false;
}
	...
	switch (error) {
		...
		*/
		case kREXError_NotEnoughMemoryForDLL:
			sprintf(text,"%s","Couldn't load the REX DLL because there is too little free memory. Quit some other application, then try again.");
			break;
		case kREXError_DLLTooOld:
			sprintf(text,"%s","Couldn't load the REX DLL because it is too old. Please update the DLL, then try again.");
			break;
		case kREXError_DLLNotFound:
			sprintf(text,"%s","The REX DLL could not be found. Maybe REX is not installed on this system?");
			break;
		case kREXError_APITooOld:
			sprintf(text,"%s","The REX DLL could not be used because it is newer than this application is expecting. Please update the application, or install an older version of the REX DLL.");
			break;
		case kREXError_UnableToLoadDLL:
			sprintf(text,"%s","Couldn't load the REX DLL. The DLL file may be corrupt. Please re-install REX, then try again. If it still doesn't work after that, call tech support.");
			break;
		...
		case kREXError_OSVersionNotSupported:
			sprintf(text,"%s","Couldn't load the REX DLL because it requires a newer version of Windows or Mac OS than is installed on this machine.");
			break;
		...
		*/
		case kREXImplError_DLLNotLoaded:
			sprintf(text,"%s","The REX DLL has not been loaded. You must call REXLoadDLL() before any other function in the REX API.");
			break;
		case kREXImplError_DLLAlreadyLoaded:
			sprintf(text,"%s","The REX DLL is already loaded. You may not nest calls to REXLoadDLL().");
			break;
