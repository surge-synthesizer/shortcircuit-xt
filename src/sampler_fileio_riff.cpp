...
size_t RIFF_StoreZone(sample_zone *z, void *data, int SampleID)
{
	...
	size_t subsize =	2*(12 + 8 + sizeof(RIFF_FILTER)) + mm_entries*(8 + sizeof(RIFF_MM_ENTRY)) + nc_entries*(8 + sizeof(RIFF_NC_ENTRY)) +
						2*(8 + sizeof(RIFF_AHDSR)) + 3*(8 + sizeof(RIFF_LFO)) + 3*(8 + sizeof(RIFF_AUX_BUSS)) +
						z->n_hitpoints*(8 + sizeof(RIFF_HITPOINT)) + (8 + sizeof(RIFF_ZONE)) + memfile::RIFFTextChunkSize(z->name);
							
	size_t zonesize = 12 + subsize;	
	
	if(!data) return zonesize;
...
size_t RIFF_StorePart(sample_part *p, void *data)
{
	...
	for(int i=0; i<n_custom_controllers; i++) ctrltxtsize += memfile::RIFFTextChunkSize(p->userparametername[i]);
	
	size_t subsize =	2*(12 + 8 + sizeof(RIFF_FILTER)) + mm_part_entries*(8 + sizeof(RIFF_MM_ENTRY)) + /*nc_entries*(8 + sizeof(RIFF_NC_ENTRY)) +*/
						num_layers * (12 + num_layer_ncs*(sizeof(RIFF_NC_ENTRY) + 8)) + 
						n_custom_controllers * (12 + 8 + sizeof(RIFF_CONTROLLER)) + ctrltxtsize +
						3*(8 + sizeof(RIFF_AUX_BUSS)) +	(8 + sizeof(RIFF_PART)) + memfile::RIFFTextChunkSize(p->name);
	
	size_t partsize = 12 + subsize;	
	
	if(!data) return partsize;
	...
	{
		mf.RIFFCreateLISTHeader('Ctrl', sizeof(RIFF_CONTROLLER) + 8 + mf.RIFFTextChunkSize(p->userparametername[i]));		
...
{	
	...
	if(IsMulti) datasize += (12 + MultiSize);
	if (chunkDataPtr) free(chunkDataPtr);
	...
	if (IsMulti)
	{
		...
		{
			mf.RIFFCreateLISTHeader('Fltr',sizeof(RIFF_FILTER) + 8 + sizeof(RIFF_FILTER_BUSSEX) + 8);
	...
	{
		HANDLE hf = CreateFileW(filename, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, 0 , NULL);
		if(!hf) goto abort;		
...
bool sampler::LoadAllFromRIFF(void* data, size_t datasize, bool Replace, int PartID)
{
	...
	bool IsMulti;
	//if((mf.RIFFGetFileType() == 'SC2P') && mf.riff_descend_RIFF_or_LIST('SC2P',0)) IsMulti = false;		
	if((mf.RIFFGetFileType() == 'SC2P') && mf.RIFFDescendSearch('SC2P')) IsMulti = false;			
	else if((mf.RIFFGetFileType() == 'SC2M') && mf.RIFFDescendSearch('SC2M')) IsMulti = true;
	else if(*(int*)data == 'mx?<') return load_all_from_xml(data,datasize,0,Replace,PartID);
	else return false;
	...
	{
		if(IsMulti) free_all();
		else part_init(PartID,true,true);
	}
	...
	{	
		...
		while(mf.RIFFDescendSearch('Part',&PartSize))
		{
			...
			while(mf.RIFFPeekChunk(&tag,&chunksize,&IsLIST,&LISTtag))
			{
				...
				{					
						...
						if(mf.RIFFDescend())
						{
							...
							{								
								if(tag == 'FltD')
								{
									...
									if (FilterID<2) ReadChunkFltD(f,&p->Filter[FilterID]);										
						...
						if(mf.RIFFDescend())
						{
							...
							{								
								if(tag == 'NCen')
								{
									...
									if (NCID<num_layer_ncs) ReadChunkNCen(nc,&p->nc[NCID + LayerID*num_layer_ncs],&mmpart);
									NCID++;
								} else mf.RIFFSkipChunk();
							}
							LayerID++;
					...
					case 'Ctrl':						
						if(mf.RIFFDescend())
						{
							while(mf.RIFFPeekChunk(&tag,&chunksize) && (CtrlID < n_custom_controllers))
							{	
								if (tag == 'CtrD')
								{
									RIFF_CONTROLLER *Ctrl = (RIFF_CONTROLLER*)mf.RIFFReadChunk();
									if (CtrlID < n_custom_controllers) ReadChunkCtrD(Ctrl,p,CtrlID);
								} 
								else if (tag == 'Name')
								{
					...
					case 'MMen':
						{
							...
							if (MMID<mm_part_entries) ReadChunkMMen(mm,&p->mm[MMID],&mmpart);
							MMID++;
						}
						break;
					case 'AuxB':
						{
							...
							if (AuxBID<3) ReadChunkAuxB(e,&p->aux[AuxBID]);
							AuxBID++;
						}
						break;
					default:					
			...
			mf.RIFFAscend();			
			PartIDMulti++;
			if((PartIDMulti>15) || !IsMulti) break;
		}
		mf.RIFFAscend(true);
	}
				...
				while(mf.RIFFPeekChunk(&tag,&chunksize,&IsLIST,&LISTtag))
				{
						...
						case 'Fltr':
							...
							{										
								while(mf.RIFFPeekChunk(&tag,&chunksize) && (FilterID < 2))
								{
									...
									{
										RIFF_FILTER *f = (RIFF_FILTER*)mf.RIFFReadChunk();
										if (FilterID<2) ReadChunkFltD(f,&z->Filter[FilterID]);
									} else mf.RIFFSkipChunk();
								}									
								FilterID++;
								mf.RIFFAscend();
					...
					else
					{
						...
						case 'MMen':
							{
								RIFF_MM_ENTRY *mm = (RIFF_MM_ENTRY*)mf.RIFFReadChunk();							
								if (MMID<mm_entries) ReadChunkMMen(mm,&z->mm[MMID],&mmzone);
								MMID++;
							}
							break;
						case 'NCen':
							{
								RIFF_NC_ENTRY *nc = (RIFF_NC_ENTRY*)mf.RIFFReadChunk();							
								if (NCID<nc_entries) ReadChunkNCen(nc,&z->nc[NCID],&mmzone);
								NCID++;
							}
							break;
						case 'AHDS':
							{
								...
								if (EGID<2) ReadChunkAHDS(e,EGID ? &z->EG2 : &z->AEG);
								EGID++;
							}
							break;
						case 'SLFO':
							{
								...
								if (LFOID<3) ReadChunkSLFO(e,&z->LFO[LFOID]);
								LFOID++;
							}
							break;
						case 'AuxB':
							{
								RIFF_AUX_BUSS *e = (RIFF_AUX_BUSS*)mf.RIFFReadChunk();							
								if (AuxBID<3) ReadChunkAuxB(e,&z->aux[AuxBID]);
								AuxBID++;
							}
							break;
						case 'HtPt':
							{
								...
								if (HtPtID<max_hitpoints) ReadChunkHtPt(e,&z->hp[HtPtID]);
								HtPtID++;
							}
							break;
						default:							
