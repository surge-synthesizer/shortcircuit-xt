...
LP4M_sat::LP4M_sat(float *fp, int *ip) : filter(fp,0,true,ip) , pre_filter(4,true), post_filter(4,true)
{
	...
	strcpy(ctrllabel[2], "drive");			ctrlmode[2] = cm_mod_decibel;		
...
void LP4M_sat::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	assert(datainL);
	assert(datainR);
	assert(param);	
	double gg = limit_range((440.0*(double)note_to_pitch(12.0*param[0])*(double)samplerate_inv*0.5),0.0,0.25);
	
	double t_b1 = 1.0 - exp(-2.0*M_PI*gg);
	g.newValue(t_b1);	
	double q = min(2.15*limit_range((double)param[1],0.0,1.0),0.5/(t_b1*t_b1*t_b1*t_b1));	
	r.newValue(q);		
...
void LP4M::process(float *data, float pitch)
{
	assert(data);
	assert(param);	
	double gg = limit_range((440*pow((double)2,(double)param[0])*(double)samplerate_inv*0.5),0,0.25);
