...
size_t sample::SaveWaveChunk(void* data)
{
	...
	size_t datasize = (8 + sizeof(wavheader)) + (8 + samplesize) + (12 + memfile::RIFFTextChunkSize(name));

	if(!data) return datasize;
...
{	
	...
	if(!skip_riffchunk && !mf.riff_descend_RIFF_or_LIST('WAVE',&datasize)) return false;	

	wavheader wh;	
	off_t wr = mf.TellI();
	if(!mf.riff_descend('fmt ',&datasize)) return false;
			
	// read header
	mf.Read(&wh,sizeof(wavheader));			

	if(!wh.nSamplesPerSec) return false;
	if(!wh.nChannels) return false;
	if(!wh.wBitsPerSample) return false;		
	...
	if(!SetMeta(wh.nChannels,wh.nSamplesPerSec,WaveDataSamples)) return false;		
	...
	if(wh.wFormatTag == WAVE_FORMAT_PCM)
	{
		if(wh.wBitsPerSample == 8)
		{
			...
			else load_data_ui8(0,loaddata,WaveDataSamples,1);
		}
		else if(wh.wBitsPerSample == 16)
		{
			...
			else load_data_i16(0,loaddata,WaveDataSamples,2);			
		}
		else if(wh.wBitsPerSample == 24)
		{
			...
			else load_data_i24(0,loaddata,WaveDataSamples,3);
		}
		else if(wh.wBitsPerSample == 32)
		{
			...
			else load_data_i32(0,loaddata,WaveDataSamples,4);
		}
		else return false;
	}
	else if(wh.wFormatTag == WAVE_FORMAT_IEEE_FLOAT)
	{
		...
		{		
			...
			else load_data_f32(0,loaddata,WaveDataSamples,4);
		...
		{		
			...
			else load_data_f64(0,loaddata,WaveDataSamples,8);
		}
		else return false;
	}	
	else return false;	
	...
	if(mf.riff_descend('smpl',&datasize))
	{
		...
		if (smpl_chunk.cSampleLoops > 0)
		{
			...
			if(smpl_loop.dwType==1) meta.playmode = pm_forward_loop_bidirectional;
	...
	{		
		...
		if(!(ACID.type & 0x01)) meta.n_beats = ACID.n_beats;
	}
	...
	{		
		...
		if(cuepoints>1)
		{
			...
			for(int i=0; i<cuepoints; i++)
			{
				...
				if(i) meta.slice_end[i-1] = cp.dwSampleOffset;
			}
		}
	}
	else
		...
		{		
			...
			if(subchunks > 1)
			{
				...
				for(int i=0; i<subchunks; i++)
				{
					...
					if(i) meta.slice_end[i-1] = entry.spos1;
				}
			}
		}
	}	
