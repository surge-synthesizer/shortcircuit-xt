...
// rev 4 will have (amplitude^2 replaced as dB's as well as exponential lfo & envelope times) LOTS of changes
// rev 5: slices have endpoints as well.. the version field stores the actual version
// rev 6: sawtooth osc amplitude scales better with unison count..
// rev 7: ringmod & freqshift have amount controls
// rev 8: c0-c15 are now stored WITHOUT the name of the controller (as originally intended), sf2 relative addressing are now delimited with a '|' sign instead of a comma
// rev 8: was also saving effect-id's incorrectly
// rev 9: SF2 now actually(!) stores files as |, 
...
bool sampler::load_file(const char *file_name,int *new_g, int *new_z, bool *is_group, char channel, int add_zones_to_groupid, bool replace)
{
	char filename[256];
	...
	if((!stricmp(extension,"wav")) || (!stricmp(extension,"aif")) || (!stricmp(extension,"aiff")) || (!stricmp(extension,"rcy")) || (!stricmp(extension,"rex")) || (!stricmp(extension,"rx2")))
	{
	...
	else if((!stricmp(extension,"gig")) || (!stricmp(extension,"dls")) || (!stricmp(extension,"sc2p")) || (!stricmp(extension,"sc2m")) || (!stricmp(extension,"sfz")))
	{
		...
		*last = 0;
		...
		HANDLE hf = CreateFileW(wfilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
		if(!hf) return false;
...
{
	if (!str) return 0;
	if (stricmp(str,"yes") == 0) return 1;
	return 0;
}

void sampler::recall_zone_from_element(TiXmlElement &element, sample_zone *zone, int revision, configuration *conf)
{
	assert(conf != 0);
	double d;
	int i;
	...
	element.Attribute("layer",&i);			zone->layer = i;
	element.Attribute("key_root",&i);		zone->key_root = i;
	element.Attribute("key_low",&i);		zone->key_low = i;
	element.Attribute("key_high",&i);		zone->key_high = i;
	element.Attribute("lo_vel",&i);			zone->velocity_low = i;
	element.Attribute("hi_vel",&i);			zone->velocity_high = i;	

	element.Attribute("key_low_fade",&i);		zone->key_low_fade = i;
	element.Attribute("key_high_fade",&i);		zone->key_high_fade = i;
	element.Attribute("lo_vel_fade",&i);		zone->velocity_low_fade = i;
	element.Attribute("hi_vel_fade",&i);		zone->velocity_high_fade = i;
	
	//element.Attribute("part",&i);				zone->part = i;		// don't store (stored in the parent instead)
	element.Attribute("output",&i);			zone->aux[0].output = i;
	if (element.QueryIntAttribute("aux_output",&i) == TIXML_SUCCESS) zone->aux[1].output = i;
	else zone->aux[1].output = -1;	

	element.Attribute("transpose",&i);		zone->transpose = i;
	element.Attribute("finetune",&d);		zone->finetune = (float)d;
	element.Attribute("pitchcorrect",&d);	zone->pitchcorrection = (float)d;
	
	tstring = element.Attribute("playmode");
	zone->playmode = pm_forward;
	for(i=0; i<n_playmodes; i++)
		if(!strcmp(tstring,playmode_abberations[i])) zone->playmode = i;
	
	element.Attribute("velsense",&d);		zone->velsense = (float)d;

	if (element.QueryDoubleAttribute("keytrack",&d) == TIXML_SUCCESS) zone->keytrack = (float)d;
	else zone->keytrack = 1;

	element.Attribute("sample_start",&i);	zone->sample_start = i;
	element.Attribute("sample_end",&i);		zone->sample_stop = i;
	element.Attribute("loop_start",&i);		zone->loop_start = i;
	element.Attribute("loop_end",&i);		zone->loop_end = i;
	element.Attribute("loop_crossfade_length",&i);		zone->loop_crossfade_length = i;	
	
	element.Attribute("amplitude",&d);		zone->aux[0].level = (float)d;
	if (element.QueryDoubleAttribute("aux_level",&d) == TIXML_SUCCESS) zone->aux[1].level = (float)d;
	element.Attribute("pan",&d);			zone->aux[0].balance = (float)d;
	element.Attribute("prefilter_gain",&d);	zone->pre_filter_gain = (float)d;
	element.Attribute("PB_depth",&i);		zone->pitch_bend_depth = i;

	element.Attribute("mute_group",&i);		zone->mute_group = i;
	element.Attribute("ignore_polymode",&i);	zone->ignore_part_polymode = i;
//	element.Attribute("polymode",&i);		zone->polymode = i;
//	element.Attribute("portamode",&i);		zone->portamento_mode = i;
//	if (element.QueryDoubleAttribute("portamento",&d) == TIXML_SUCCESS) zone->portamento = (float)d;
//	else zone->portamento = -10.f;	
...
//	element.Attribute("ef_release",&d);			zone->ef_release = (float)d;
	element.Attribute("lag0",&d);				zone->lag_generator[0] = (float)d;
	element.Attribute("lag1",&d);				zone->lag_generator[1] = (float)d;
	...
	while(sub)
	{
		...
		if(j<3)
		{
			if(sub->QueryDoubleAttribute("level",&d) == TIXML_SUCCESS) zone->aux[j].level = d;
			if(sub->QueryDoubleAttribute("balance",&d) == TIXML_SUCCESS) zone->aux[j].balance = d;
			if(sub->QueryIntAttribute("output",&i) == TIXML_SUCCESS) zone->aux[j].output = i;
			if(j && (sub->QueryIntAttribute("outmode",&i) == TIXML_SUCCESS)) zone->aux[j].outmode = i;
		}
	...
	while (sub){
		...
		if(j<2)
		{
			...
			if(revision<2)
			{
				...
				if ((i>=0) && (i<14)) vtCopyString(abb, filter_abberations_beta1[i], 16);
				else strcpy(abb,"NONE");
			}
			else
			{
			...
			{
				if(strcmp(filter_abberations[l],abb) == 0) ft = l;
			}			
			zone->Filter[j].type = ft;

			if(sub->QueryDoubleAttribute("mix",&d) == TIXML_SUCCESS) zone->Filter[j].mix = d;
...
// TODO restore morphEQ import
				...
				{											
					if (sub->QueryDoubleAttribute(label,&d) == TIXML_SUCCESS)	zone->Filter[j].p[k] = (float)d;
				}
			}
			...
			{					
				...
				if (ft == ft_fx_ringmod) zone->Filter[j].p[2] = 1.f;
			}			
		}
	...
	while (sub){
		...
		if(j<2)
		{
			sub->Attribute("attack",&d); 		env[j]->attack = (float)d;
			sub->Attribute("attack_shape",&d); 	env[j]->shape[0] = (float)d;
			sub->Attribute("hold",&d); 			env[j]->hold = (float)d;
			sub->Attribute("decay",&d); 		env[j]->decay = (float)d;
			sub->Attribute("decay_shape",&d); 	env[j]->shape[1] = (float)d;
			sub->Attribute("sustain",&d); 		env[j]->sustain = (float)d;
			sub->Attribute("release",&d); 		env[j]->release = (float)d;
			sub->Attribute("release_shape",&d); env[j]->shape[2] = (float)d;
	...
	while(sub){
		...
		if(j<3)
		{
			sub->Attribute("rate",&d);			zone->LFO[j].rate			= (float)d;
			sub->Attribute("smooth",&d);		zone->LFO[j].smooth			= (float)d;
			sub->Attribute("cyclemode",&i);		zone->LFO[j].cyclemode		= i;
			sub->Attribute("triggermode",&i);	zone->LFO[j].triggermode	= i;
			sub->Attribute("temposync",&i);		zone->LFO[j].temposync		= i;
			sub->Attribute("repeat",&i);		zone->LFO[j].repeat			= i;
			sub->Attribute("once",&i);		zone->LFO[j].onlyonce			= i;
			int k;
			for(k=0; k<32; k++)
			{
				char label[9];
				sprintf(label,"step%i",k);
				if (sub->QueryDoubleAttribute(label,&d) == TIXML_SUCCESS)	zone->LFO[j].data[k] = (float)d;				
			}
		}
	...
	while(sub)
	{
		...
		{			
			...
			sub->Attribute("amount",&d);	zone->mm[id].strength	= (float)d;
	...
	while(sub)
	{
        ...
        if(id<max_hitpoints)
		{
			...
			sub->Attribute("samplepos",&i);		zone->hp[id].start_sample	= i;			
			sub->Attribute("env",&d);			zone->hp[id].env			= (float)d;
			...
			else
			{
				...
				if (sub) 
				{
					sub->Attribute("samplepos",&i);		zone->hp[id].end_sample	= i;
				}
				else zone->hp[id].end_sample = zone->sample_stop;
			}
		}
	}
}
...
void sampler::store_zone_as_element(TiXmlElement &element, sample_zone *zone, configuration *conf)
{
	...
	for(int i=0; i<3; i++)
	{
		...
		if(i) output.SetAttribute("outmode",zone->aux[i].outmode);
...
//	element.SetAttribute("ef_release",float_to_str(zone->ef_release,tempstr));
	...
	for(i=0; i<2; i++)
	{
		...
		if (zone->Filter[i].type) element.InsertEndChild(filter);
	}	
...
void store_part_as_element(TiXmlElement &element, sample_part *part, configuration *conf)
{
	...
	if (part->transpose) element.SetAttribute("transpose",part->transpose);	
	if (part->formant) element.SetAttribute("formant",part->formant);	
	...
	if (part->portamento_mode) element.SetAttribute("portamode",part->portamento_mode);
	if (part->vs_layercount) element.SetAttribute("vs_layers",part->vs_layercount);
	...
	for(int i=0; i<num_layers; i++)
	{
		...
		if (used) element.InsertEndChild(layer);
	}	 
	...
	for(int i=0; i<3; i++)
	{
		...
		if(i) output.SetAttribute("outmode",part->aux[i].outmode);
		element.InsertEndChild(output);
	}
	...
	for(int i=0; i<2; i++)
	{
		...
		if(part->Filter[i].type) element.InsertEndChild(filter);
	}	
	...
	{						
		...
		if (part->userparameter[i] || part->userparametername[i][0] || part->userparameterpolarity[i])	// one of them has to differ from the default to be stored
...
void recall_part_from_element(TiXmlElement &element, sample_part *part, int revision, configuration *conf, bool include_outputs, bool include_userparam_state)
{
	assert(conf != 0);
	double d;
	int i;
	...
	element.Attribute("transpose",&i);		part->transpose = i;
	element.Attribute("formant",&i);		part->formant = i;
	element.Attribute("channel",&i);		part->MIDIchannel = i;
	element.Attribute("poly_limit",&i);		part->polylimit = i;
	element.Attribute("polymode",&i);		part->polymode = i;

	element.Attribute("pfg",&d);			part->pfg = (float)d;
	element.Attribute("portamento",&d);		part->portamento = (float)d;
	element.Attribute("portamode",&i);		part->portamento_mode = i;

	element.Attribute("vs_layers",&i);			part->vs_layercount = i;
	element.Attribute("xf_equality",&i);		part->vs_xf_equality = i;
	element.Attribute("vs_distribution",&d);	part->vs_distribution = (float)d;
	element.Attribute("vs_xfade",&d);			part->vs_xfade = (float)d;
	...
	if(include_outputs)
	{
		...
		while(sub)
		{
			...
			if(j<3)
			{
				if(sub->QueryDoubleAttribute("level",&d) == TIXML_SUCCESS) part->aux[j].level = d;
				if(sub->QueryDoubleAttribute("balance",&d) == TIXML_SUCCESS) part->aux[j].balance = d;
				if(sub->QueryIntAttribute("output",&i) == TIXML_SUCCESS) part->aux[j].output = i;
				if(j && (sub->QueryIntAttribute("outmode",&i) == TIXML_SUCCESS)) part->aux[j].outmode = i;
			}
			sub = sub->NextSibling("output")->ToElement();
		}
	}
	...
	while (sub){
		...
		if(j<2)
		{
			...
			if(sub->QueryDoubleAttribute("mix",&d) == TIXML_SUCCESS) part->Filter[j].mix = d;
			...
			for(k=0; k<n_filter_parameters; k++)
			{
				char label[9];
				sprintf(label,"p%i",k);	
				if (sub->QueryDoubleAttribute(label,&d) == TIXML_SUCCESS)	part->Filter[j].p[k] = (float)d;				
			}						
			for(k=0; k<n_filter_iparameters; k++)
			{
				char label[9];
				sprintf(label,"ip%i",k);	
				if (sub->QueryIntAttribute(label,&i) == TIXML_SUCCESS)	part->Filter[j].ip[k] = i;				
			}	
		}
		sub = sub->NextSibling("filter")->ToElement();
	}
	...
	while(sub)
	{
		...
		{			
			...
			sub->Attribute("amount",&d);	part->mm[id].strength	= (float)d;
...
{	
	...
	if (hFind != INVALID_HANDLE_VALUE) 
	{
		...
		{						
			...
			else if((FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && !(FindFileData.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT))
			{				
				string subdir = path;				
...
//	double d;
	...
	if (sc->QueryIntAttribute("revision",&revision) != TIXML_SUCCESS) revision = 1;
	...
	{		
		...
		if(part_id != -1) p_id = part_id;
		else part->Attribute("i",&p_id);
		...
		{			
			...
			if (zone->Attribute("filename"))
			{
				...
				is_loaded = get_sample_id(samplefname,&sample_id);		// ACHTUNG!! blir fuck-up om filename inte finns (?)

				if(is_loaded)
				{
					if (sample_id>=0) samples[sample_id]->remember();
				...
				else 
				{
					...
					else
					{
						...
						if(r) vtCopyString(filename,r+1,256);
						else vtCopyString(filename,samplefname,256);
						...
						while(!dont_ask_path)
						{
							...
							char title[256];
							sprintf(title,"The file [%s] could not be found. Select the directory wherein it is located. (will search subdirs)",filename);
							char np[MAX_PATH];
							bi.lpszTitle = title;
...
bool sampler::load_all_from_sc1_xml(void* data, int datasize, char *filename, bool replace, int part_id)
{
	int i;
	int revision;
	//	double d;
