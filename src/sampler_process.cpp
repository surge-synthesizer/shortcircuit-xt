...
{	
	if(id == out_part) return output_part[(part<<1) + channel];
	else if(id >= out_fx1) return output_fx[(((id-out_fx1)&0x7)<<1) + channel];
	//else return output_ptr[(((id-out_output1)&0x7)<<1)+channel];
	// HACK, disablad no-memcpy speedup
	else return output[(((id-out_output1)&0x7)<<1)+channel];
}
...
{	
	...
	if(partv[p].last_ft[f] != parts[p].Filter[f].type)
	{
		...
		if(partv[p].pFilter[f]) partv[p].pFilter[f]->init();
...
{	
	...
	{		
		...
		if(multiv.last_ft[f] != multi.Filter[f].type)
		{
			...
			multiv.pFilter[f] = spawn_filter(multi.Filter[f].type,multi.Filter[f].p,multi.Filter[f].ip,0,true);		
			if(multiv.pFilter[f]) multiv.pFilter[f]->init();
...
{	
	...
	{
		float b = fabs(controllers_target[p*n_controllers + i] - controllers[p*n_controllers + i]);
		float a = 0.1*b;
		controllers[p*n_controllers + i] = (1-a)*controllers[p*n_controllers + i] + a*controllers_target[p*n_controllers + i];	
	}
	for(unsigned int  i=0; i<n_custom_controllers; i++)
	{
		float b = fabs(parts[p].userparameter[i] - parts[p].userparameter_smoothed[i]);
		float a = 0.1*b;
		parts[p].userparameter_smoothed[i] = (1-a)*parts[p].userparameter_smoothed[i] + a*parts[p].userparameter[i];	
	}	
	...
	partv[p].pfg.set_target(db_to_linear(partv[p].mm->get_destination_value(md_part_prefilter_gain)));	
...
{	
	...
	if(holdengine)
	{		
		for(unsigned int op=0; op<(mNumOutputs); op++) clear_block(output[op],block_size_quad<<1);
		return;
	}
	...
	if (sample_replace_filename[0])
	{					
		if(zone_exist(selected->get_active_id()) && (selected->get_active_type() == 1)) kill_notes(selected->get_active_id());
	}
		
	//memset(output,0,sizeof(output));
	for(unsigned int op=0; op<(mNumOutputs<<1); op++) clear_block_antidenormalnoise(output_ptr[op],block_size_quad);
	for(unsigned int op=0; op<(n_sampler_effects<<1); op++) clear_block(output_fx[op],block_size_quad);
	for(unsigned int op=0; op<(n_sampler_parts<<1); op++) clear_block(output_part[op],block_size_quad);
	...
	{				
		...
		{												
			float *outbuf[3][2];			
			outbuf[0][0] = get_output_pointer(voices[v]->zone->aux[0].output,0,voices[v]->zone->part);
			outbuf[0][1] = get_output_pointer(voices[v]->zone->aux[0].output,1,voices[v]->zone->part);
			outbuf[1][0] = get_output_pointer(voices[v]->zone->aux[1].output,0,voices[v]->zone->part);
			outbuf[1][1] = get_output_pointer(voices[v]->zone->aux[1].output,1,voices[v]->zone->part);
			outbuf[2][0] = get_output_pointer(voices[v]->zone->aux[2].output,0,voices[v]->zone->part);
			outbuf[2][1] = get_output_pointer(voices[v]->zone->aux[2].output,1,voices[v]->zone->part);			
			
			voice_state[v].active = voices[v]->process_block(outbuf[0][0],outbuf[0][1],outbuf[1][0],outbuf[1][1],outbuf[2][0],outbuf[2][1]);
