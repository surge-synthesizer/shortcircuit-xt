	...
	*/
	...
	else
	{
		...
		if(n_ph>2)
		{
			...
			pre_id = spawn_patch_dialog((HWND)((AEffGUIEditor*)editor)->getFrame()->getSystemWindow(), mp, n_ph-1);
			delete mp;		
		}
#endif*/
	}
	...
	{				
		...
		int pg,
			pg_first = preset_bag[pb].wGenNdx,
			pg_end = preset_bag[pb+1].wGenNdx;

		bool is_global_zone = ((pb==pb_first)&&(preset_gen[min(pg_end,n_pg)-1].sfGenOper != instrument));
		...
		else
		{
			...
			if (!p_generators_set[instrument]) break;
			...
			{					
				int ig,
					ig_first = inst_bag[ib].wInstGenNdx,
					ig_end = inst_bag[ib+1].wInstGenNdx;

				bool is_inst_global_zone =  ((ib==ib_first)&&(inst_gen[min(ig_end,n_ig)-1].sfGenOper != sampleID));
				...
				{										
					...
					memcpy(i_generators_set,i_globalzone_generators_set,sizeof(bool[sf2_numgenerators]));					

					for(ig=ig_first; ig<min(ig_end,n_ig); ig++)	
					{
						int sfgo = inst_gen[ig].sfGenOper;
						if(sfgo >= sf2_numgenerators) break;
					...
					for(p=0; p<sf2_numgenerators; p++)
					{
						...
						case velRange:
							...
							{
								i_generators[p].ranges.byLo = max(i_generators[p].ranges.byLo,p_generators[p].ranges.byLo);
								i_generators[p].ranges.byHi = min(i_generators[p].ranges.byHi,p_generators[p].ranges.byHi);
					...
					if (i_generators_set[sampleID] &&
						((shdr[sample_id].sfSampleType == monoSample)||(shdr[sample_id].sfSampleType == rightSample)) &&
						add_zone(fn,&newzone,channel))
					{
						...
						z->mute			= false;
						//z->layer = (pb-pb_first) & 7;		// layer by preset-bag test (no good)
						...
						if (z->key_root > 127) z->key_root = 60;
						...
						if(i_generators_set[overridingRootKey] && (i_generators[overridingRootKey].shAmount >= 0) && (i_generators[overridingRootKey].shAmount < 128))
							z->key_root = (char)i_generators[overridingRootKey].shAmount;

						if (i_generators[sampleModes].wAmount & 1) z->playmode = pm_forward_loop;
						else z->playmode = pm_forward;
						...
						z->AEG.decay = timecent_to_envtime(i_generators[decayVolEnv].shAmount) * egmult;
						z->AEG.release = timecent_to_envtime(i_generators[releaseVolEnv].shAmount) * egmult;
						...
						z->LFO[0].rate = log2(8.176f * powf(2,i_generators[freqModLFO].shAmount/1200.f));	// mod lfo
						z->LFO[0].triggermode = 0;
						z->LFO[1].rate = log2(8.176f * powf(2,i_generators[freqVibLFO].shAmount/1200.f));	// vib lfo
						...
						if (i_generators[modLfoToFilterFc].shAmount)
						{
							...
							z->mm[mmslot++].strength = i_generators[modLfoToFilterFc].shAmount/1200.f;
						}
						if (i_generators[modEnvToFilterFc].shAmount)
						{
							z->mm[mmslot].source = get_mm_source_id("EG2");
							z->mm[mmslot].destination = get_mm_dest_id("f1p1");
							z->mm[mmslot++].strength = i_generators[modEnvToFilterFc].shAmount/1200.f;
						}
						...
						if (i_generators[keynumToVolEnvHold].shAmount)
						{
							...
							z->mm[mmslot++].strength = 0.01f*i_generators[keynumToVolEnvHold].shAmount;
						}
						if (i_generators[keynumToVolEnvDecay].shAmount)
						{
							z->mm[mmslot].source = get_mm_source_id("keytrack");
							z->mm[mmslot].destination = get_mm_dest_id("eg1d");
							z->mm[mmslot++].strength = 0.01f*i_generators[keynumToVolEnvDecay].shAmount;
						}
						if (i_generators[keynumToModEnvHold].shAmount)
						{
							z->mm[mmslot].source = get_mm_source_id("keytrack");
							z->mm[mmslot].destination = get_mm_dest_id("eg2h");
							z->mm[mmslot++].strength = 0.01f*i_generators[keynumToModEnvHold].shAmount;
						}
						if (i_generators[keynumToModEnvDecay].shAmount)
						{
							z->mm[mmslot].source = get_mm_source_id("keytrack");
							z->mm[mmslot].destination = get_mm_dest_id("eg2d");
							z->mm[mmslot++].strength = 0.01f*i_generators[keynumToModEnvDecay].shAmount;
						}
						...
						if (shdr[sample_id].sfSampleType == monoSample) z->aux[0].balance = 0.002f * i_generators[pan].shAmount;
						else z->aux[0].balance = 0.f;

						

						if (i_generators_set[initialFilterFc] && (i_generators[initialFilterFc].wAmount > 1200))
