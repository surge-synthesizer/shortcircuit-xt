...
SuperSVF::SuperSVF(float *fp, int *ip) 
: filter(fp,0,true,ip)
, mPolyphase(2,true)
{
...
void SuperSVF::ProcessT(float *DataInL, float *DataInR, float *DataOutL, float *DataOutR, float Pitch)
{
	calc_coeffs();
	...
	for (int k=0; k<block_size; k++)
	{
		...
		if (Stereo)
		{
			if (FourPole)
			{
				...
				_mm_store_ss(&PolyphaseInR[(k<<1)], _mm_shuffle_ps(MiddleOutput, MiddleOutput, _MM_SHUFFLE(3,3,3,3)));
				_mm_store_ss(&PolyphaseInL[(k<<1)+1], _mm_movehl_ps(LastOutput, LastOutput));
				_mm_store_ss(&PolyphaseInR[(k<<1)+1], _mm_shuffle_ps(LastOutput, LastOutput, _MM_SHUFFLE(3,3,3,3)));
			}
			else
			{
				_mm_store_ss(&PolyphaseInL[(k<<1)], MiddleOutput);				
				_mm_store_ss(&PolyphaseInR[(k<<1)], _mm_shuffle_ps(MiddleOutput, MiddleOutput, _MM_SHUFFLE(1,1,1,1)));
				_mm_store_ss(&PolyphaseInL[(k<<1)+1], LastOutput);				
				_mm_store_ss(&PolyphaseInR[(k<<1)+1], _mm_shuffle_ps(LastOutput, LastOutput, _MM_SHUFFLE(1,1,1,1)));
			}
		}
		else
		{
	...
	mPolyphase.process_block_D2(
		PolyphaseInL, 
		Stereo ? PolyphaseInR : PolyphaseInL, 
		bs2, 
		DataOutL, 
...
void SuperSVF::process_stereo(float *DataInL, float *DataInR, float *DataOutL, float *DataOutR, float Pitch)
{
	if (iparam[1]>0)
	{
