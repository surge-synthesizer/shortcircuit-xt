...
void vg_rect::move_centerpivot(int x, int y) { move(x - (get_w() >> 1), y - (get_h() >> 1)); }
...
{
    return (((r.x < x2) && (r.x2 > x)) || ((x < r.x2) && (x2 > r.x))) &&
           (((r.y < y2) && (r.y2 > y)) || ((y < r.y2) && (y2 > r.y)));
}
...
vg_surface::~vg_surface() { assert(imgdata == 0); }
...
bool vg_surface::create(int sizeX, int sizeY, bool DIB)
{
    ...
    sizeXA = sizeX;
    if (sizeXA & 3)
        sizeXA = (sizeXA & 0xFFFFFFFC) + 4; // make every row alignable (16-bytes)
    sizeYA = sizeY;
...
void vg_surface::fill_ramp(vg_rect r, unsigned int color, int quadrant)
{
    ...
    if (vertical)
    {
        ...
        if (quadrant & 2)
        {
            ...
            {
                int x1 = r.x2 - (h >> 16);
                for (int x = x1; x < r.x2; x++)
                    alphablend_pixel(color, imgdata[y * sizeXA + x]);
                int alpha = ((color >> 24) & 0xff) * ((h >> 8) & 0xff);
                alphablend_pixel(color & 0x00ffffff | ((alpha << 16) & 0xff000000),
                                 imgdata[y * sizeXA + max(0, x1 - 1)]);
                h += dh;
            }
        }
        else
    ...
    else
    {
        ...
        else
        {
            ...
            {
                int y1 = r.y2 - (h >> 16);
                for (int y = y1; y < r.y2; y++)
                    alphablend_pixel(color, imgdata[y * sizeXA + x]);
                int alpha = ((color >> 24) & 0xff) * ((h >> 8) & 0xff);
                alphablend_pixel(color & 0x00ffffff | ((alpha << 16) & 0xff000000),
                                 imgdata[max(0, y1 - 1) * sizeXA + x]);
                h += dh;
            }
        }
    }
...
void vg_surface::set_HDC(HDC nhdc) { hdc = nhdc; }
...
void vg_surface::borderblit(vg_rect r, vg_bitmap source)
{
    ...
    // draw edges
    {
        ...
        for (int y = 0; y < h; y++)
        {
            ...
            unsigned int c =
                source.surf->imgdata[(cw + source.r.x) + (y2 + source.r.y) * source.surf->sizeXA];
            for (int x = cw; x < we; x++)
            {
        ...
        for (int x = 0; x < w; x++)
        {
            ...
            unsigned int c =
                source.surf->imgdata[(x2 + source.r.x) + (ch + source.r.y) * source.surf->sizeXA];
            for (int y = ch; y < he; y++)
            {
                alphablend_pixel(c, imgdata[(x + r.x) + (y + r.y) * sizeXA]);
            }
    ...
    // fill middle
    {
        unsigned int midcolor =
            source.surf->imgdata[(cw + source.r.x) + (ch + source.r.y) * source.surf->sizeXA];
        vg_rect r2 = r;
        r2.x += cw;
        r2.x2 -= cw;
        r2.y += ch;
...
void vg_surface::draw_text_multiline(vg_rect r, std::string text, unsigned int color, int halign,
                                     int valign)
{
    if (!font)
        return;
...
void vg_surface::draw_text(vg_rect r, std::string text, unsigned int color, int halign, int valign,
                           int ibeampos, bool IgnoreParsing)
{
    assert(font);
    if (!font)
        return;
    ...
    {
        for (int y = 0; y < h; y++)
            fontblend(color, 255, dst[max(0, x - 1) + y * sizeXA]);
    }
    ...
    for (int i = 0; i < text.size(); i++)
    {
        ...
        {
            for (int y = 0; y < h; y++)
                fontblend(color, 255, dst[x + y * sizeXA]);
        }
...
vg_font::vg_font() { data = 0; }

//-------------------------------------------------------------------------------------------------------

vg_font::~vg_font() { delete data; }
...
bool vg_font::load(std::string filename)
{
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        {
            unsigned int a =
                (surf.imgdata[x + (i & 0xf) * cell_width + (i >> 4) * surfheight * surf.sizeXA]) &
                0xff;
            if (!a)
            {
                char_width[i] = x;
        ...
        for (int y = 0; y < cell_height; y++)
        {
            for (int x = 0; x < cell_width; x++)
            {
                unsigned int a = (surf.imgdata[x + (i & 0xf) * cell_width +
                                               (y + 1 + (i >> 4) * surfheight) * surf.sizeXA]) &
                                 0xff;
...
vg_control::vg_control(vg_window *o, int id, TiXmlElement *e) : surf(o)
{
    owner = o;
    control_id = id;
    parameter_id = 0;
...
vg_control::~vg_control() { surf.destroy(); }

//-------------------------------------------------------------------------------------------------------

void vg_control::draw() { surf.clear(0x80ff0000); }
...
std::string vg_control::decode_vars(std::string s)
{
    ...
    if (pos != std::string::npos)
    {
        ...
        if (offset)
            sid += offset_amount * owner->filters[offset];
        sprintf(meh, "%i", sid);
        s.replace(pos, 2, meh);
    }
    return s;
...
void vg_control::draw_background()
{
    ...
    else if (owner->background.is_initiated())
    {
        vg_bitmap b;
        b.r = location;
...
{
    if ((filter > 0) && (filter < max_vgwindow_filters) &&
        !((1 << owner->filters[filter]) & filter_entry))
        return false;
    return true;
}
