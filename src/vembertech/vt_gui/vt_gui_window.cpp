...
{
    return ((m & MK_LBUTTON) ? vgm_LMB : 0) | ((m & MK_RBUTTON) ? vgm_RMB : 0) |
           ((m & MK_MBUTTON) ? vgm_MMB : 0) | ((m & MK_XBUTTON1) ? vgm_MB4 : 0) |
           ((m & MK_XBUTTON2) ? vgm_MB5 : 0) | ((GetKeyState(VK_MENU) < 0) ? vgm_alt : 0) |
           ((m & MK_SHIFT) ? vgm_shift : 0) | ((m & MK_CONTROL) ? vgm_control : 0);
}
...
LRESULT __declspec(dllexport) __stdcall CALLBACK
    hookKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    char ch;
    if (kbd_hook_hWnd && (HC_ACTION == nCode))
    {
        ...
        if (appwin)
        {
            ...
            e.buttonmask = (GetKeyState(VK_SHIFT) ? vgm_shift : 0) |
                           (GetKeyState(VK_CONTROL) ? vgm_control : 0);
        ...
        if ((appwin->keyboardmode == kbm_os) || (appwin->keyboardmode == kbm_seperate_window))
        {
            ...
            if (ToAscii(vkey, scancode, (PBYTE)ks, (LPWORD)tb, 0) == 1)
            {
                ...
                if (lParam & 0x40000000)
                    e.eventtype = vget_keyrepeat; // if repeating, change eventtype
...
bool vg_window::create(std::string filename, bool is_editor, vg_window *owner, void *syswindow)
{
    ...
    for (int i = 0; i < max_vgwindow_filters; i++)
        filters[i] = 0;
    ...
    if (syswindow && (keyboardmode != kbm_seperate_window)) // host has already created a window
    {
        ...
        hWnd = CreateWindowEx(0, ("vgwindow"), ("Window"), WS_CHILD | WS_VISIBLE, 0, 0, sizeX,
                              sizeY, (HWND)syswindow, NULL, wc.hInstance, this);
    ...
    else
    {
        ...
        // Create the application's window
        hWnd = CreateWindowEx(
            is_editor ? WS_EX_TOOLWINDOW : 0, is_editor ? ("vgwindow_e") : ("vgwindow"),
            is_editor ? ("vember GUI editor") : ("shortcircuit2"), WS, is_editor ? 1150 : 100, 100,
            sizeX, sizeY, owner ? owner->hWnd : GetDesktopWindow(), NULL, wc.hInstance, this);
    ...
    if (keyboardmode == kbm_globalhook)
    {
        kbd_hook_hWnd = hWnd;
        gkbdhook =
            SetWindowsHookEx(WH_KEYBOARD, (HOOKPROC)hookKeyboardProc, (HINSTANCE)hInstance, 0);
    }
...
void vg_window::destroy()
{
    ...
    DestroyWindow(hWnd);
    /*DeleteObject(hbmp);
    // NOTE: this->surf does not need to be destroyed since it's freed by DeleteObject. so just set
    the pointer to 0 surf.imgdata = 0;*/
    surf.destroy();

    UnregisterClass(is_editor ? ("vgwindow_e") : ("vgwindow"), GetModuleHandle(NULL));

    if (background.is_initiated())
        background.destroy();

    for (int i = 0; i < 8; i++)
    {
        if (altbg[i].is_initiated())
            altbg[i].destroy();
    }
    delete menu;
}
...
void vg_window::draw_child(int id, bool redraw, HDC hdc)
{
    ...
    if ((children[id]->filter &&
         !((1 << filters[children[id]->filter]) & children[id]->filter_entry)))
        return;
    ...
    {
        if ((selected_id == id) ||
            (multiselector.size() && (multiselector.find(id) != multiselector.end())))
        {
...
void vg_window::draw(void *hwnd, int ctrlid)
{
    ...
    if (editmode)
    {
        ...
        if (editwin)
        {
            editwin->toolbox_selectedstate(selected_id >= 0, multiselector.size() > 1,
                                           editmode_tool);
            if (selected_id >= 0)
            {
    ...
    if (ctrlid >= 0)
    {
        ...
        BitBlt(hdc, R.x, R.y, min(R.x2, surf.sizeXA), min(R.y2, surf.sizeYA), (HDC)hdcWinBuf, R.x,
               R.y, SRCCOPY);
    }
    else if (GetUpdateRect((HWND)hwnd, &UR, false))
        BitBlt(hdc, UR.left, UR.top, min(UR.right, surf.sizeXA), min(UR.bottom, surf.sizeYA),
               (HDC)hdcWinBuf, 0, 0, SRCCOPY);
    else
...
bool vg_window::processevent(vg_controlevent &e)
{
    ...
    if (editmode)
    {
            ...
            else
            {
                ...
                for (int i = 1; i < 10; i++)
                {
                    if ((sizer_rects[i].point_within(p)) && (selected_id >= 0))
                    {
                        ...
                        if ((i == 3) || (i == 6) || (i == 9))
                            editmode_drag_origin.x -= sizer_rects[0].x2;
                        else
                            editmode_drag_origin.x -= sizer_rects[0].x;

                        if ((i == 1) || (i == 2) || (i == 3))
                            editmode_drag_origin.y -= sizer_rects[0].y2;
                        else
                            editmode_drag_origin.y -= sizer_rects[0].y;
        ...
        {
            if (sizer_rects[2].point_within(p))
                set_cursor(cursor_sizeNS);
            else if (sizer_rects[8].point_within(p))
                set_cursor(cursor_sizeNS);
            else if (sizer_rects[4].point_within(p))
                set_cursor(cursor_sizeWE);
            else if (sizer_rects[6].point_within(p))
                set_cursor(cursor_sizeWE);
            else if (sizer_rects[1].point_within(p))
                set_cursor(cursor_sizeNESW);
            else if (sizer_rects[9].point_within(p))
                set_cursor(cursor_sizeNESW);
            else if (sizer_rects[3].point_within(p))
                set_cursor(cursor_sizeNWSE);
            else if (sizer_rects[7].point_within(p))
                set_cursor(cursor_sizeNWSE);
            else if (sizer_rects[5].point_within(p))
            ...
            if (editmode_drag == 5)
            {
                ...
                if (e.buttonmask & vgm_shift)
                {
                    ...
                    {
                        cr.move_constrained(editmode_cursor_origin.x - editmode_drag_origin.x,
                                            p.y - editmode_drag_origin.y, get_rect());
                    }
                    else
                    {
                        cr.move_constrained(p.x - editmode_drag_origin.x,
                                            editmode_cursor_origin.y - editmode_drag_origin.y,
                                            get_rect());
                    }
                }
                else
                {
                    cr.move_constrained(p.x - editmode_drag_origin.x, p.y - editmode_drag_origin.y,
                                        get_rect());
                }
            ...
            else if (editmode_drag > 0)
            {
                ...
                int i = editmode_drag;
                if ((i == 1) || (i == 4) || (i == 7))
                    cr.x = min(p.x, cr.x2 - children[selected_id]->get_min_w());
                if ((i == 3) || (i == 6) || (i == 9))
                    cr.x2 = max(p.x, cr.x + children[selected_id]->get_min_w());
                if ((i == 7) || (i == 8) || (i == 9))
                    cr.y = min(p.y, cr.y2 - children[selected_id]->get_min_h());
                if ((i == 1) || (i == 2) || (i == 3))
                    cr.y2 = max(p.y, cr.y + children[selected_id]->get_min_h());
        ...
        if ((e.eventtype == vget_mousemove) && (e.buttonmask & vgm_LMB) && !editmode_drag)
        {
            ...
            for (unsigned int i = 0; i < children.size(); i++)
            {
                if (children[i]->visible())
                {
                    bool b;
                    if (picky)
                        b = sel.does_encase(children[i]->get_rect());
                    else
                        b = sel.does_intersect(children[i]->get_rect());
                    if (b)
                    {
                        multiselector.insert(i);
                    }
            ...
            if ((selected_id < 0) && (multiselector.size() > 0))
                set_selected_id(*multiselector.begin());
        }
    }
    ...
    else if (!dirtykids.empty())
    {
        ...
        if (dirtykids.size() < 2)
        {
            ...
            if (!((*iter >= 0) && (*iter < (int)children.size())))
                RedrawWindow(hWnd, 0, 0, RDW_INVALIDATE);
            else
            {
...
void vg_window::save_layout()
{
    ...
    if (background_filename.size())
        layout.SetAttribute("background", background_filename.c_str());

    for (int i = 0; i < 7; i++)
    {
        char atr[64];
        sprintf(atr, "background%i", i);
        if (altbg_filename[i].size())
            layout.SetAttribute(atr, altbg_filename[i].c_str());
    }
    ...
    for (unsigned int i = 0; i < n; i++)
    {
        ...
        if (children[i]->parameter_subid)
            control.SetAttribute("parameter_subid", children[i]->parameter_subid);
...
bool vg_window::load_layout()
{
    ...
    TiXmlDocument doc;
    if (!doc.LoadFile(layoutfile.c_str()))
        return false;
    ...
    if (bgfile)
    {
        if (!background.create(rootdir_skin + bgfile))
            background.create(rootdir_default + bgfile);
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        if (bgfile)
        {
            if (!altbg[i].create(rootdir_skin + bgfile))
                altbg[i].create(rootdir_default + bgfile);
    ...
    while (control)
    {
        ...
        if (rect)
        {
            ...
            if (rect->QueryIntAttribute("x", &j) == TIXML_SUCCESS)
                x = j;
            if (rect->QueryIntAttribute("x2", &j) == TIXML_SUCCESS)
                x2 = j;
            if (rect->QueryIntAttribute("y", &j) == TIXML_SUCCESS)
                y = j;
            if (rect->QueryIntAttribute("y2", &j) == TIXML_SUCCESS)
                y2 = j;
            ...
            if (t)
            {
                ...
                if (c)
                    t->parameter_id = param_get_id_from_label(c);
                if ((t->parameter_id < 0) &&
                    (control->QueryIntAttribute("parameter", &j) == TIXML_SUCCESS))
                    t->parameter_id = j;
                if (control->QueryIntAttribute("parameter_subid", &j) == TIXML_SUCCESS)
                    t->parameter_subid = j;
                if (control->QueryIntAttribute("filter", &j) == TIXML_SUCCESS)
                    t->filter = j;
                const char *s = control->Attribute("filter_entry");
                if (s)
                    t->filter_entry = bitmask_from_hexlist(s);

                if (control->QueryIntAttribute("offset", &j) == TIXML_SUCCESS)
                    t->offset = j;
                if (control->QueryIntAttribute("offset_amount", &j) == TIXML_SUCCESS)
                    t->offset_amount = j;
...
vg_bitmap vg_window::get_art(unsigned int bank_id, unsigned int style_id, unsigned int entry_id)
{
    ...
    {
        int j, b_id = 0;
        if (bank->QueryIntAttribute("i", &j) == TIXML_SUCCESS)
            b_id = j;
        if (b_id == bank_id)
        {
            int bs = 0;
                    ...
                    while (surface)
                    {
                        ...
                        if (s)
                        {
                            ...
                            while (bitmap)
                            {
                                ...
                                {
                                    int x = 0, x2 = 0, y = 0, y2 = 0;
                                    if (bitmap->QueryIntAttribute("x", &j) == TIXML_SUCCESS)
                                        x = j;
                                    if (bitmap->QueryIntAttribute("x2", &j) == TIXML_SUCCESS)
                                        x2 = j;
                                    if (bitmap->QueryIntAttribute("y", &j) == TIXML_SUCCESS)
                                        y = j;
                                    if (bitmap->QueryIntAttribute("y2", &j) == TIXML_SUCCESS)
                                        y2 = j;
                                    vg_bitmap nb;
                                    nb.r.set(x, y, x2, y2);
                                    nb.surf = s;
...
void vg_window::load_art()
{
        ...
        while (style)
        {
            ...
            while (surface)
            {
                ...
                if (fn)
                {
                    ...
                    if (s->read_png(rootdir_skin + fn) == -1)
                        s->read_png(rootdir_default + fn);
    ...
    while (font)
    {
        ...
        if (!nf->load(rootdir_skin + fn))
            nf->load(rootdir_default + fn);
...
void vg_window::set_cursor(int a)
{
    ...
    case cursor_move:
        ...
        break;
    case cursor_copy:
        if (hcursor_arrowcopy)
            SetCursor(hcursor_arrowcopy);
        break;
    case cursor_zoomin:
        if (hcursor_zoomin)
            SetCursor(hcursor_zoomin);
        break;
    case cursor_zoomout:
        if (hcursor_zoomout)
            SetCursor(hcursor_zoomout);
        break;
    case cursor_vmove:
        if (hcursor_vmove)
            SetCursor(hcursor_vmove);
        break;
    case cursor_hmove:
        if (hcursor_hmove)
            SetCursor(hcursor_hmove);
        break;
    case cursor_text:
...
void vg_window::toolbox_selectedstate(bool selected, bool multiselector, int currenttool)
{
    ...
    ad.id = editor_alignV_top;
    post_action_to_control(ad);
    ad.id = editor_alignV_mid;
    post_action_to_control(ad);
    ad.id = editor_alignV_bottom;
    post_action_to_control(ad);
    ad.id = editor_alignH_left;
    post_action_to_control(ad);
    ad.id = editor_alignH_center;
    post_action_to_control(ad);
    ad.id = editor_alignH_right;
    post_action_to_control(ad);
    ad.id = editor_scaleV;
    post_action_to_control(ad);
    ad.id = editor_scaleH;
...
bool vg_window::delete_child(int id) // only works one at a time (since the ids will change)
{
    if (id < 0) // delete all selected children
    {
        if (multiselector.size() > 0)
        {
            if ((selected_id >= 0) && (multiselector.find(selected_id) != multiselector.end()))
                multiselector.insert(selected_id);
            int n = (int)children.size();
            for (int i = 0; i < n; i++)
            {
...
void vg_window::events_from_toolbox(actiondata ad)
{
    ...
    else if (ad.actiontype == vga_text)
    {
        ...
        case editor_rectY2:
        {
            for (std::set<int>::iterator iter = multiselector.begin(); iter != multiselector.end();
                 iter++)
            {
                int i = *iter;
                assert((i >= 0) && (i < (int)children.size()));
...
void vg_window::post_action_from_program(actiondata ad)
{
    ...
    if (initialized)
        PostMessage(hWnd, WM_APP + 1212, 0, 0);
}
...
void vg_window::process_actions_from_program()
{
    ...
    for (unsigned int i = 0; i < children.size(); i++)
    {
        ...
        {
            children[i]->parameter_subid_tempoverride +=
                children[i]->offset_amount * filters[children[i]->offset];
        }
    }
    int last_pid = -1;
...
void vg_window::post_action(actiondata ad, vg_control *c)
{
    ...
    else if (is_editor && owner)
    {
        if (c)
        {
            ...
            if (c->offset)
                ad.subid += c->offset_amount * filters[c->offset];
        }
        owner->events_from_toolbox(ad);
    }
    else
    {
        if ((ad.actiontype == vga_deletezone) &&
            !dialog_confirm(
                L"Delete zones?",
                L"This operation will remove the selected zone(s) from the part.\n\nAre you sure?",
                0))
            return;
...
int vg_window::get_syscolor(int id) { return colorpalette[id & 0xff]; }
...
void vg_window::dialog_save_part()
{
    ...
    if (savepart_fname.empty())
        return;

    actiondata ad;
    memset(&ad, 0, sizeof(actiondata));
    ad.actiontype = vga_save_patch;
    /*ad.data.i[0] = dialog_confirm(L"Copy samples with patch?",L"Do you wish to make a copy of the
    samples to the location of the patch?\n\nChoosing 'No' will make the patch reference the
    contained files at their original location.",3); if (ad.data.i[0]) ad.data.i[1] =
    dialog_confirm(L"Update references?",L"Do you wish to update the in-memory references to the
    newly created copies?",3); else ad.data.i[1] = 0;*/
    post_action_to_program(ad);
}
