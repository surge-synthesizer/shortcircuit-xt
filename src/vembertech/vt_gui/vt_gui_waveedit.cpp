...
vg_waveeditor::vg_waveeditor(vg_window *o, int id, TiXmlElement *e) : vg_control(o, id, e)
{
    ...
    calc_aatable(aatable[0], owner->get_syscolor(col_standard_bg),
                 owner->get_syscolor(col_standard_text));
    calc_aatable(aatable[1], owner->get_syscolor(col_standard_text),
                 owner->get_syscolor(col_standard_bg));
    calc_aatable(aatable[2], 0xff4ea1f3, 0xff000000);

    for (int i = 0; i < 7; i++)
        bmpdata[i] = owner->get_art(vgct_waveedit, 0, i);

    restore_data(e);
}

vg_waveeditor::~vg_waveeditor() { wavesurf.destroy(); }

int vg_waveeditor::getpos(int sample) { return ((sample - start) / max(0, zoom)) + wmargin; }

int vg_waveeditor::getsample(int pos) { return (pos - wmargin) * zoom + start; }
...
void vg_waveeditor::draw()
{
    ...
    case 1:
        // multiple zones selected
        {
            ...
            surf.draw_text(r, "Multiple zones selected. Changes will be applied to all of them.",
                           owner->get_syscolor(col_standard_text));
            set_dirty();
        }
        break;
...
void vg_waveeditor::draw_wave(bool be_quick, bool skip_wave_redraw)
{
    ...
    vg_rect r2 = rwave;
    r2.x2 = r2.x + wmargin;
    wavesurf.fill_rect(r2, margincol);
    r2 = rwave;
    r2.x = r2.x2 - wmargin;
    wavesurf.fill_rect(r2, margincol);
    ...
    if (!skip_wave_redraw)
        for (int c = 0; c < s->channels; c++)
        {
            ...
            { // set start conditions. "last value" must be right to avoid line-drawing at the left
              // edge
                float val;
                if (s->UseInt16)
                {
            ...
            for (int x = 0; x < imgw; x += (debugzoom ? 4 : 1))
            {
                ...
                else
                {
                    for (int sp = 0; sp < ratio; sp += sample_inc)
                    {
                        ...
                        int cpos = pos + sp;
                        //					if (pm_has_loop(playmode)&&(cpos >=
                        // loop_start)&&(cpos < loop_end)) aaidx = 2;
                        float val;
                        if (s->UseInt16)
                        {
                        ...
                        if (debugzoom)
                        {
                            ...
                            {
                                img[y * imgwa + x] =
                                    aatable[aaidx][min((unsigned int)255, column[y] >> aa_p2)];
    ...
    case cs_pan:
    {
        ...
        else if (e.eventtype == vget_mousemove)
        {
            ...
            if (so)
            {
                ...
                {
                    float movemult = 2.f;
                    if (e.buttonmask & (vgm_RMB | vgm_control))
                        movemult = 8.f;
