...
bool browserdata::valid_extension(int ft, const wchar_t *extension, bool &is_bank)
{
    ...
    {
    case 0:
        if ((wcsicmp(extension, L".wav") == 0) || (wcsicmp(extension, L".aif") == 0) ||
            (wcsicmp(extension, L".aiff") == 0) /*||(wcsicmp(extension,L".rx2") == 0)*/)
            return true;
        break;
    case 1:
        ...
        {
            is_bank = (wcsicmp(extension, L".sf2") == 0) || (wcsicmp(extension, L".dls") == 0) ||
                      (wcsicmp(extension, L".gig") == 0);
            return true;
        }
        break;
    };
...
bool browserdata::build_samplelist(int ft, database_samplelist *s, int n_samples)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        {
            database_samplelist *sptr =
                (database_samplelist *)((char *)s + j * sizeof(database_samplelist));
        ...
        sprintf(es.name, "%s [%.1fMB]", (i == 0) ? "Embedded" : "Referenced",
                (float)csize / (1024.f * 1024.f));
                        ...
                        /*out*/ LPWSTR lpszFilePath)
{
    ...
    if (SUCCEEDED(hRes))
    {
        ...
        // #if !defined _UNICODE
        // 		MultiByteToWideChar(CP_UTF8, 0, lpszShortcutPath,		// var
        // CP_ACP
        //                                        -1, wszTemp, MAX_PATH);
        // #else
            ...
            if (hFind != INVALID_HANDLE_VALUE)
            {
                while (1)
                {
                    ...
                    if (extension && (wcsicmp(extension, L".lnk") == 0))
                    {
                        ...
                        if (!FAILED(hr))
                            attacher = traverse_dir(i, szFilePath, 1, attacher, 0);
                    }
...
bool browserdata::inject_newfile(int type, std::wstring filename)
{
    ...
    for (int c = 0; c < n; c++)
    {
        if (_strnicmp(categorylist[type][c].path, pathUTF8, 256) == 0)
        {
            ...
            {
                categorylist[type][cid].patch_id_end =
                    max(categorylist[type][cid].patch_id_end, categorylist[type][c].patch_id_end);
...
int browserdata::traverse_dir(int ftype, std::wstring dir, int depth, int creator, int parent)
{
    ...
    e.child = -1;
    e.sibling = -1;
    if (categorylist[ftype][creator].depth == depth)
        categorylist[ftype][creator].sibling = e.id;
    else
        categorylist[ftype][creator].child = e.id;
    e.depth = depth;
                ...
                if (extension)
                {
                    ...
                    if (valid_extension(ftype, extension, is_bank))
                    {
                        if (is_bank)
                        {
                            ...
                            midipatch *plist = 0;
                            int n;
                            if (wcsicmp(extension, L".sf2") == 0)
                                n = get_sf2_patchlist(sf2file.c_str(), (void **)&plist);
                            else
                                n = get_dls_patchlist(sf2file.c_str(), (void **)&plist);
                            for (int i = 0; i < n; i++)
                            {
                                patch_entry f;
                                WCHAR s[256];
                                ...
                                std::wstring spath;
                                if ((n > 1) && (i > 0))
                                    spath = dir + FindFileData.cFileName + L">" + s;
                                else
                                    spath = dir + FindFileData.cFileName;
                                WideCharToMultiByte(CP_UTF8, 0, spath.c_str(), -1, f.path, 256, 0,
                                                    0);
                                f.category = e.id;
                                ...
                                f.filetype = ftype;
                                WideCharToMultiByte(CP_UTF8, 0, extension, -1, f.column2, 16, 0, 0);
                                _strlwr_s(f.column2, 16);
                        ...
                        else
                        {
                            ...
                            *extension = 0;
                            ...
                            WideCharToMultiByte(CP_ACP, 0, FindFileData.cFileName, -1, f.name, 64,
                                                0, 0);
                            patchlist[ftype].push_back(f);
                        }
                    }
    ...
    /*if(depth && (categorylist[ftype][e.id].patch_id_end ==
    categorylist[ftype][e.id].patch_id_start))
    {
...
void browserdata::save_cache()
{
    ...
    int i;
    i = categorylist[0].size();
    fwrite(&i, 4, 1, f);
    i = patchlist[0].size();
    fwrite(&i, 4, 1, f);

    i = categorylist[1].size();
    fwrite(&i, 4, 1, f);
    i = patchlist[1].size();
    fwrite(&i, 4, 1, f);

    i = categorylist[2].size();
    fwrite(&i, 4, 1, f);
    i = patchlist[2].size();
    fwrite(&i, 4, 1, f);

    i = categorylist[3].size();
    fwrite(&i, 4, 1, f);
    i = patchlist[3].size();
    fwrite(&i, 4, 1, f);
...
bool browserdata::load_cache()
{
    ...
    for (int s = 0; s < 4; s++)
    {
        ...
        {
            category_entry ce;
            if (!fread(&ce, sizeof(category_entry), 1, f))
                return false;
        ...
        {
            patch_entry pe;
            if (!fread(&pe, sizeof(patch_entry), 1, f))
                return false;
