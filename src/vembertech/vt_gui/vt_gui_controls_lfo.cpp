...
void vg_steplfo::draw()
{
    ...
    for (int i = 0; i < n_steps; i++)
    {
        ...
        steprect[i] = r2;
        surf.fill_rect(r2,
                       (i & 1) ? owner->get_syscolor(col_LFOBG1) : owner->get_syscolor(col_LFOBG2));
        ...
        for (int x = 0; x < ((r2.get_w() + 1) << 4); x++)
        {
            ...
            if (y < surf.sizeY)
                alphablend_pixel(((31 - fr) << 24) | (0x00ffffff & col),
                                 surf.imgdata[surf.sizeXA * y + r2.x + (x >> 4)]);
            if ((y + 1) < surf.sizeY)
                alphablend_pixel((fr << 24) | (0x00ffffff & col),
                                 surf.imgdata[surf.sizeXA * (y + 1) + r2.x + (x >> 4)]);
        }
    }
}
...
void vg_steplfo::processevent(vg_controlevent &e)
{
    ...
    if ((e.eventtype == vget_mousedown) ||
        ((e.eventtype == vget_mousemove) && (e.buttonmask & (vgm_LMB | vgm_RMB))))
    {
        if (e.eventtype == vget_mousedown)
        ...
        for (int i = 0; i < n_steps; i++)
        {
            if ((e.x >= steprect[i].x) && (e.x < steprect[i].x2))
            {
                ...
                if (e.buttonmask & vgm_RMB)
                    ...
                    step[i] = min(1.f, max(-1.f, 1.f - 2.f * f));
