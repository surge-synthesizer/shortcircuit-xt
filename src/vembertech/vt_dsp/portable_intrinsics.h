...
forceinline vFloat vLoad(float *f) { return vec_ld(0, f); }

forceinline vFloat vSqrtFast(vFloat v) { return vec_re(vec_rsqrte(v)); }
...
forceinline vFloat vLoad1(float f) { return _mm_load1_ps(&f); }

forceinline vFloat vSqrtFast(vFloat v) { return _mm_rcp_ps(_mm_rsqrt_ps(v)); }
