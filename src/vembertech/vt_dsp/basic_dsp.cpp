...
unsigned int Min(unsigned int a, unsigned int b) { return min(a, b); }
unsigned int Max(unsigned int a, unsigned int b) { return max(a, b); }

int limit_range(int x, int l, int h) { return max(min(x, h), l); }
...
__m128 sine_xpi_ps_SSE2(__m128 x) // sin(x*pi)
{
