...
inline void createGroupIn(int partNumber, engine::Engine &engine, MessageController &cont)
{
...

    {
        ...
        undo::pushUndo<undo::GroupsDeleteOnUndoItem>(
            engine, std::vector<std::pair<int16_t, int32_t>>{{(int16_t)partNumber, gi}});
    }
...
inline void removeZone(const selection::SelectionManager::ZoneAddress &a, engine::Engine &engine,
                       MessageController &cont)
{
    undo::pushUndo<undo::ZonesRestoreItem>(
        engine, std::vector<selection::SelectionManager::ZoneAddress>{a});
...
inline void removeSelectedZones(const bool &, engine::Engine &engine, MessageController &cont)
{
    ...
    undo::pushUndo<undo::ZonesRestoreItem>(
        engine, std::vector<selection::SelectionManager::ZoneAddress>{zs.begin(), zs.end()});
                        ...
                        MessageController &cont)
{
    ...
    undo::pushUndo<undo::GroupOrderRestoreItem>(engine, src.part);
...
{
    {
        ...
        if (!affected.empty())
            undo::pushUndo<undo::PartsStateItem>(engine, affected);
    }
