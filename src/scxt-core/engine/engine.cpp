                                                ...
                                                bool useSampleRangeInfo, bool forceSynchronous)
{
    ...
    // Push an undo step for the group before modifying it. A multi-file drop
    // (one add message per file) is coalesced to one entry by the UI bracketing
    // the loop with begin/endCoalescing.
    undo::pushUndo<undo::GroupChangeItem>(*this, sp, sg);
...
void Engine::copyZones(const std::vector<selection::SelectionManager::ZoneAddress> &addrs)
{
    ...
    // a zone never renamed shows a derived name, which the pasted copy would not inherit
         ...
         sortedValid(addrs, [this](const auto &a) { return isValidZoneAddress(a); }))
    {
        ...
        names.push_back(zones.back()->getName());
    }
    if (zones.empty())
        return;
    ...
    messaging::client::serializationSendToClient(
        ...
        clipboard.streamToClipboard(Clipboard::ContentType::ZONE, zones, names),
        *messageController);
}

void Engine::pasteZone(const selection::SelectionManager::ZoneAddress &aIn)
{
    if (clipboard.getClipboardType() != Clipboard::ContentType::ZONE)
        return;

    // -1 for either means "wherever a new zone would go", as createEmptyZone reads it
    auto a = aIn;
    if (a.part < 0 || a.group < 0)
    {
        auto [bp, bg] = selectionManager->bestPartGroupForNewSample(*this);
        if (a.part < 0)
            a.part = bp;
        if (a.group < 0)
            a.group = bg;
    ...
    for (size_t i = 0; i < clipboard.getClipboardItemCount(); ++i)
    {
        ...
        if (!clipboard.unstreamFromClipboard(Clipboard::ContentType::ZONE, i, *zptr))
            continue;
        zptr->givenName = takeFreeCopyName(clipboard.getClipboardItemName(i), zoneNames);
...
void Engine::sendFullRefreshToClient() const
{
    ...
    messaging::client::serializationSendToClient(messaging::client::s2c_update_mpe_tuning_awareness,
                                                 ...
                                                 *getMessageController());
    messaging::client::serializationSendToClient(
        messaging::client::s2c_update_pitchbend_tuning_awareness,
        runtimeConfig.tuningAwarePitchBends, *getMessageController());
...
void Engine::setPBTuningAwareness(bool a) { this->runtimeConfig.tuningAwarePitchBends = a; }
