                          ...
                          const VelocityRange &vr)
{
    ...
    if (iDim & VEL_RANGE_START)
    {
        ...
        doApply = doApply && (hasFeature::geometryChangesAdjustFade ||
                              (vr.velEnd - vr.velStart - deltaY >= vr.fadeStart + vr.fadeEnd));
    }
    ...
    if (iDim & KEY_RANGE_START)
    {
        ...
        doApply = doApply && (hasFeature::geometryChangesAdjustFade ||
                              (kr.keyEnd - kr.keyStart - deltaX >= kr.fadeStart + kr.fadeEnd));
    }
    ...
    if (iDim & KEY_FADE_START)
    {
        doApply = doApply && (kr.fadeStart + deltaX + kr.fadeEnd <= kr.keyEnd - kr.keyStart);
        doApply = doApply && (kr.fadeStart + deltaX >= 0);
    }
    if (iDim & KEY_FADE_END)
    {
        doApply = doApply && (kr.fadeStart + deltaX + kr.fadeEnd <= kr.keyEnd - kr.keyStart);
        doApply = doApply && (kr.fadeEnd + deltaX >= 0);
    }
    if (iDim & VEL_FADE_START)
    {
        doApply = doApply && (vr.fadeStart + deltaY + vr.fadeEnd <= vr.velEnd - vr.velStart);
        doApply = doApply && (vr.fadeStart + deltaY >= 0);
    }
    if (iDim & VEL_FADE_END)
    {
        doApply = doApply && (vr.fadeStart + deltaY + vr.fadeEnd <= vr.velEnd - vr.velStart);
        doApply = doApply && (vr.fadeEnd + deltaY >= 0);
    }
                                     ...
                                     const KeyboardRange &kr, const VelocityRange &vr)
{
    ...
    {
        return newX + kr.fadeEnd <= (kr.keyEnd - kr.keyStart);
    }
    if (dim == KEY_FADE_END)
    {
        return kr.fadeStart + newX <= (kr.keyEnd - kr.keyStart);
    }
    if (dim == VEL_FADE_START)
    {
        return newY + vr.fadeEnd <= (vr.velEnd - vr.velStart);
    }
    if (dim == VEL_FADE_END)
    {
        return vr.fadeStart + newY <= (vr.velEnd - vr.velStart);
    }
    return true;
}
