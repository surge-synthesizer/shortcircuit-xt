    ...
    : scxt::modulation::shared::ProcessorTargetEndpointData<TG, 'gprc'>(p)
{
    if (e)
    {
        ...
        auto ptShortFn = [](const engine::Group &z,
                            const GroupMatrixConfig::TargetIdentifier &t) -> std::string {
            auto &d = z.processorDescription[t.index];
            return shared::processorSlotShortPath(d, t.index, d.type == dsp::processor::proct_none);
        };
        ...
        for (int i = 0; i < scxt::maxProcessorFloatParams; ++i)
        {
            ...
            registerGroupModTarget(e, fpT[i], ptFn, elFn, adFn, ptShortFn, elShortFn, enFn);
        }

        order.separator();
        registerGroupModTarget(
            e, mixT, ptFn, mixFn, [](const auto &, const auto &) -> int32_t { return false; },
            ptShortFn, mixShortFn, slotFilledFn);
        registerGroupModTarget(
            e, outputLevelDbT, ptFn, levFn,
            [](const auto &, const auto &) -> int32_t { return true; }, ptShortFn, levShortFn,
            slotFilledFn);
    }
}
