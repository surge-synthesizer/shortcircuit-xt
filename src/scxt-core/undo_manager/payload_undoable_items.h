 ...
 *   static void serialExtra(const engine::Engine &); // extra UI refresh
 *
...
template <typename S>
concept SpecHasSerialExtra = requires(const engine::Engine &e) { S::serialExtra(e); };
template <typename S>
concept SpecHasSerialExtraAddr =
    requires(const engine::Engine &e, const std::vector<ZoneAddress> &sel, int32_t i) {
        S::serialExtra(e, sel, i);
    };
template <typename S>
    ...
    void restore(engine::Engine &e) override
    {
        ...
        e.getMessageController()->scheduleAudioThreadCallback(
            ...
            [sel, idx](const auto &eng) {
                ...
                if constexpr (SpecHasSerialExtra<Spec>)
                    Spec::serialExtra(eng);
                if constexpr (SpecHasSerialExtraAddr<Spec>)
                    Spec::serialExtra(eng, sel, idx);
            });
    }
...
struct GroupOutputInfoSpec : GroupMemberSpec<&engine::Group::outputInfo>
{
    ...
    static void serialExtra(const engine::Engine &e) { resendPGZStructure(e); }
};
...
struct BusEffectSpec
{
    ...
    static void serialExtra(const engine::Engine &e, const std::vector<ZoneAddress> &sel, int32_t);
};
