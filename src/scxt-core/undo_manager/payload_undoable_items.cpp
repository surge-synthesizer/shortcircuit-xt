...
void BusEffectSpec::extraClientRefresh(const engine::Engine &e, const std::vector<ZoneAddress> &sel,
                                       int32_t)
{
...
void BusSendSpec::extraClientRefresh(const engine::Engine &e, const std::vector<ZoneAddress> &sel,
                                     int32_t)
{
    for (const auto &a : sel)
        e.getPatch()
...
void PartConfigSpec::extraClientRefresh(const engine::Engine &e,
                                        const std::vector<ZoneAddress> &sel, int32_t)
{
    for (const auto &a : sel)
    {
        messaging::client::serializationSendToClient(
