...
void Envelope::Assign(float *envA, float *envH, float *envD, float *envS, float *envR, float *envshape){
	
	A = envA;	H = envH;	D = envD;	S = envS;	R = envR;
	shape = envshape;
...
void Envelope::SetCurve(float x)
{
	curve = (unsigned int)(curve_offset + x);
}
...
float Envelope::CalcCurve()
{
	...
	return (1.f / 65536.f) *
		(table_envshape[curve][e_coarse] * (float)(0x10000 - e_fine) +
...
{	
	...
	case sDecay :		//decay
		{			
			...
			else
			{
				...
				if(level<cut_level) state = sIdle;
			}								
		}
		break;
	case sSustain :
	...
	case sRelease :		//release
		{
			...
			if (phase > 0x80000000) state = sIdle;
	...
	if (state==sIdle) level = 0;		

	output = level;
	if (state == sIdle) return false;
	return true;
}
