...
{	
	...
	parameter_count = 2;
	strcpy(ctrllabel[0], "tune");				ctrlmode[0] = cm_mod_pitch;	
	strcpy(ctrllabel[1], "width");				ctrlmode[1] = cm_percent;
	...
	int i;
	for(i=0; i<ob_length; i++) oscbuffer[i] = 0.f;
...
void osc_pulse::convolute()
{
	...
	{
		oscbuffer[bufpos+k&(ob_length-1)] += fpol*(SincTableF32[m+k] + lipol*SincOffsetF32[m+k]);	
	}		
...
void osc_pulse::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
...
{		
	if (first_run)
	{
		...
		oscstate -= large;
		int i;
		for(i=0; i<ob_length; i++) oscbuffer[i] *= -0.5f;
		oscstate = 0;
		polarity = 0;
	}
	...
	{	
		oscstate -= large;				
		while(oscstate<0) this->convolute();
...
{	
	...
	parameter_count = 3;
	strcpy(ctrllabel[0], "tune");				ctrlmode[0] = cm_mod_pitch;		
	strcpy(ctrllabel[1], "width");				ctrlmode[3] = cm_percent;
	strcpy(ctrllabel[2], "sync");				ctrlmode[4] = cm_mod_pitch;
...
void osc_pulse_sync::convolute()
{
	...
	if(sync) polarity = false;
	
	// add time until next statechange	
	double width = (0.5 - 0.499f*min(1.f, max(0.f, param[1])));
	double t = max(0.5,samplerate / (440.0 * pow((double)1.05946309435,(double)pitch + param[0] + param[2])));
	lastpulselength = t;
	if (polarity)		
	{
		width = 1-width;
...
void osc_pulse_sync::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
	process(0,dataoutL,pitch);
	copy_block(dataoutL,dataoutR,block_size_quad);
}
...
{			
	...
	{	
		oscstate -= large;
		syncstate -= large;
		while(syncstate<0) this->convolute();
		while(oscstate<0) this->convolute();
		osc_out = osc_out*integrator_hpf + oscbuffer[bufpos];
		dataout[k] = osc_out;
		oscbuffer[bufpos] = 0.f;		
...
{	
	...
	parameter_count = 3;
	strcpy(ctrllabel[0], "tune");					ctrlmode[0] = cm_mod_pitch;		
	strcpy(ctrllabel[1], "detune");					ctrlmode[1] = cm_mod_pitch;		
	strcpy(ctrllabel[2], "ringmod");					ctrlmode[2] = cm_percent;
...
void osc_saw::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{
...
{			
	...
	{	
		...
		{
			oscstate[l] -= large;				
			while(oscstate[l]<0) this->convolute(l);
			dc += dc_uni[l];
		}
		
		oscbuffer[(bufpos+6)&(ob_length-1)] -= dc;
		osc_out = osc_out*integrator_hpf + oscbuffer[bufpos];		
		dataout[k] = out_attenuation * (1.f + param[2] * (datain[k] - 1.f)) * osc_out;	// ringmod mix
		oscbuffer[bufpos] = 0.f;
...
{				
	osc.set_rate(440.0 * pi2 * pow((double)1.05946309435,(double)pitch + param[0]) * samplerate_inv);	
...
void osc_sin::process_stereo(float *datainL, float *datainR, float *dataoutL, float *dataoutR, float pitch)
{			
	osc.set_rate(440.0 * pi2 * pow((double)1.05946309435,(double)pitch + param[0]) * samplerate_inv);	
