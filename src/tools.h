...
inline float int100_to_float(int i)
{
	return float(i) * (1.f / 100.f);
}

//----------------------------------------------------------------------------------------

inline float int127_to_float(int i)
{
	return float(i) * (1.f / 127.f);
}
...
inline double symmetric_nuttall(double i, int n)
{
	...
	i -= (n/2);
	return a0 - a1*cos(2.0*PI*i/(n)) + a2*cos(4.0*PI*i/(n)) - a3*cos(6.0*PI*i/(n));
}
