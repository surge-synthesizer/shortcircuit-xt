...
{
	//write_log("suspend");
	if (initialized) sobj->all_notes_off();
}
...
{		
	...
	if(!cs) return 0;
	*data = cs;	
	for(int i=0; i<16; i++) cs->params[i] = sobj->controllers_target[c_custom0+i];
...
long scpb_vsti::setChunk (void* data, long byteSize, bool isPreset)
{
	...
	{	
		...
		if(c == 0)
		{
			for(int j=0; j<16; j++) sobj->controllers_target[c_custom0+j] = cs->params[j];
...
void scpb_vsti::handleEvent(VstEvent* ev){
	...
	if (ev->type == kVstMidiType){		
		...
		{			
			...
			{
				int rid = sobj->channel_controller((char)channel,midiData[1] & 0x7f,midiData[2] & 0x7f);
				//if((rid>=0)&&(rid<8))
				{
...
//-------------------------------------------------------------------------------------------------------
void scpb_vsti::setProgramName (char *name)
{
	strcpy (programName, name);
}

//-----------------------------------------------------------------------------------------
void scpb_vsti::getProgramName (char *name)
{		
...
void scpb_vsti::setParameter (long index, float value)
{
	...
	if(editor) ((scpb_editor*)editor)->setParameter(index,value);
}
...
//-----------------------------------------------------------------------------------------
void scpb_vsti::getParameterLabel(long index, char *label)
{
	strcpy (label, "");
}
...
//-----------------------------------------------------------------------------------------
template<bool replacing> void scpb_vsti::processT(float **inputs, float **outputs, long sampleFrames)
{
	if (!initialized) return;
	...
	for(i=0; i<sampleFrames; i++)
    {
		...
		{
			if (replacing)
				outputs[2*output_pairs + outp][i] = sobj->output[outputs_mono_index + outp][blockpos];    // replacing	
			else
				outputs[2*output_pairs + outp][i] += sobj->output[outputs_mono_index + outp][blockpos];    // adding	
		}
		blockpos++;
...
{		
	if(!sobj) init();	
	if(sobj->currentpatch<0) return 0;
