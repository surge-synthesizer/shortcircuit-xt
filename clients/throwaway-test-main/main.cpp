...
void PrintSfInfo(sf2::File *sf)
{
    ...
    cout << "\tSound ROM Version: " << sf->pInfo->pRomVer->Major << "." << sf->pInfo->pRomVer->Minor
         << endl;
...
void PrintInstruments(sf2::File *sf)
{
    ...
    for (int i = 0; i < sf->GetInstrumentCount(); i++)
    {
        ...
        if (instr->pGlobalRegion)
            PrintRegion(-1, instr->pGlobalRegion);
...
{
    if (idx == -1)
        cout << "\t\tGlobal Region " << endl;
    else
        cout << "\t\tRegion " << idx << endl;
    sf2::Sample *s = reg->GetSample();
    if (s != NULL)
    {
        cout << "\t\t    Sample: " << s->Name << ", Fine Tune: " << reg->fineTune;
        if (reg->coarseTune)
            cout << ", Coarse Tune: " << reg->coarseTune;
        if (reg->overridingRootKey != -1)
            cout << ", Overriding Root Key: " << reg->overridingRootKey;
        if (reg->HasLoop)
        {
    ...
    cout << "\t\t    Key range=";
    if (reg->loKey == ::sf2::NONE && reg->hiKey == ::sf2::NONE)
        cout << "None";
    else
        cout << reg->loKey << "-" << reg->hiKey;
    cout << ", Velocity range=";
    if (reg->minVel == ::sf2::NONE && reg->maxVel == ::sf2::NONE)
        cout << "None";
    else
        cout << reg->minVel << "-" << reg->maxVel;
    cout << endl;
    ...
    cout << GetValue(reg->GetEG1Decay())
         << "s,  EG1Sustain=" << GetValue(reg->GetEG1Sustain() / 10);
    ...
    cout << "\t\t    Modulation LFO: Delay=" << ::sf2::ToSeconds(reg->delayModLfo)
         << "s, Frequency=";
    cout << ::sf2::ToHz(reg->freqModLfo) << "Hz, LFO to Volume=" << (reg->modLfoToVolume / 10)
         << "dB";
...
string GetControllerSource(sf2::Modulator &mod)
{
    ...
    case sf2::Modulator::PITCH_WHEEL_SENSITIVITY:
        s = "Pitch Wheel Sensitivity";
        break;
    case sf2::Modulator::LINK:
        s = "Link";
        break;
    default:
        s = "Unknown controller source";
    }
