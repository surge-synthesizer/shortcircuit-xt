...
TEST_CASE("Part config gesture coalesces to one undo", "[undo]")
{
    UndoFixture f;
    auto &part = f.engine().getPatch()->getPart(0);
    auto origPan = part->configuration.pan;
    auto baseSize = f.undoManager().undoStackSize();

    f.send(cmsg::BeginEdit({(int32_t)cmsg::EditSubtree::part_config, false, 0}));
    REQUIRE(f.undoManager().undoStackSize() == baseSize + 1);
    ...
    for (int i = 0; i < 20; ++i)
    {
        ...
        conf.pan = 0.01f * i;
        f.send(cmsg::UpdatePartFullConfig({(int16_t)0, conf}));
    }
    REQUIRE(part->configuration.pan == Approx(0.19f));
    REQUIRE(f.undoManager().undoStackSize() == baseSize + 1);

    f.sendUndo();
    REQUIRE(part->configuration.pan == Approx(origPan));

    f.sendRedo();
    REQUIRE(part->configuration.pan == Approx(0.19f));
}

TEST_CASE("Multi sample add coalesces to one undo", "[undo]")
{
    ...
    // The UI fires one add message per dropped file, bracketed by a coalescing
    // batch so a whole multi-file drop is a single undo entry.
...
TEST_CASE("Macro edits still raise wrapper param events", "[undo]")
{
    // the clap wrapper turns s2a_param_* messages into clap output events;
    // verify the macro begin/end and value sends survive the undo plumbing
    UndoFixture f;
    auto &mc = *f.engine().getMessageController();
    mc.passWrapperEventsToWrapperQueue = true;

    auto baseSize = f.undoManager().undoStackSize();
    f.send(cmsg::MacroBeginEndEdit({true, (int16_t)0, (int16_t)0}), 20);
    f.send(cmsg::SetMacroValue({(int16_t)0, (int16_t)0, 0.73f}), 20);
    f.send(cmsg::MacroBeginEndEdit({false, (int16_t)0, (int16_t)0}), 20);

    // the drag coalesced to one undo entry
    REQUIRE(f.undoManager().undoStackSize() == baseSize + 1);

    int begins{0}, ends{0}, sets{0};
    auto expectId = (uint32_t)scxt::engine::Macro::partIndexToMacroID(0, 0);
    while (!mc.engineToPluginWrapperQueue.empty())
    {
        auto m = mc.engineToPluginWrapperQueue.pop();
        if (!m.has_value())
            break;
        if (m->id == scxt::messaging::audio::s2a_param_beginendedit)
        {
            REQUIRE(m->payload.mix.u[1] == expectId);
            if (m->payload.mix.u[0])
                begins++;
            else
                ends++;
        }
        else if (m->id == scxt::messaging::audio::s2a_param_set_value)
        {
            REQUIRE(m->payload.mix.u[0] == expectId);
            REQUIRE(m->payload.mix.f[0] == Approx(0.73f));
            sets++;
        }
    }
    REQUIRE(begins == 1);
    ...
    // and the macro value itself round trips through undo
    auto &macro = f.engine().getPatch()->getPart(0)->macros[0];
    REQUIRE(macro.value == Approx(0.73f));
    f.sendUndo();
    REQUIRE(macro.value != Approx(0.73f));
}
