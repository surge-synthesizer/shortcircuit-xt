...
TEST_CASE("Mod targets on an emptied processor slot still label themselves", "[modulation]")
{
    ...
    mt::Fixture f;
    auto zt = scxt::voice::modulation::MatrixEndpoints::ProcessorTarget::floatParam(0, 3);
    auto gt = scxt::modulation::GroupMatrixEndpoints::ProcessorTarget::floatParam(0, 3);
    ...
    REQUIRE(zl == "P1.4: none");
    REQUIRE(!zen);
    REQUIRE(gl == "P1.4: none");
    REQUIRE(!gen);

    // mix and output level have no float index, so they name themselves
    using TI = scxt::modulation::shared::TargetIdentifier;
    auto zmd = scxt::voice::modulation::getVoiceMatrixMetadata(f.zone());
    REQUIRE(mt::rowLabel(zmd, TI{'proc', 'mix ', 0}).first == "P1.Mix: none");
    REQUIRE(mt::rowLabel(zmd, TI{'proc', 'oldb', 0}).first == "P1.Out Lvl: none");
}
          ...
          "[modulation]")
{
    ...
    auto expected = "P1." + std::to_string(bigCount) + ": none";
    REQUIRE(zl == expected);
    REQUIRE(!zen);
    REQUIRE(gl == expected);
    REQUIRE(!gen);
    ...
    INFO("present label '" << zl0 << "'");
    REQUIRE(zl0.rfind("P1." + f.zone().processorDescription[0].typeShortName + ": ", 0) == 0);
    REQUIRE(zl0.find("none") == std::string::npos);
}
