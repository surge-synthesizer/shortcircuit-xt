...
vg_list::vg_list(vg_window *o, int id, TiXmlElement *e) : vg_control(o,id,e)
{
	...
	for(int i=0; i<16; i++)	bmpdata[i] = owner->get_art(vgct_list,0,i);	
...
{	
	...
	{		
		if((key == z->keyroot))
		{
			int j = z - zones.begin();
			subimage = 4;
			if(z->playing) subimage = 9;
			else if(selected_id == j) subimage = 6;
			else if(multiselect.find(j) != multiselect.end()) subimage = 7;
			else if(hover_id == j) subimage = 5;	
			z->visible = true;
			z->r = r3;
			break;
		}
	...
	if(z == zones.end())
	{
		if(keyvel[key]) subimage = 15;
	}
	if(labeloverride) subimage = 13;
	surf.fill_rect(r3,owner->get_syscolor(col_list_bg));
	surf.borderblit(r3,bmpdata[subimage]);
	vg_rect r4 = r3;
	r4.inset(3,3);	
	if(labeloverride) surf.draw_text(r4,labeloverride,owner->get_syscolor(col_list_text_unassigned));
	else if(z != zones.end()) surf.draw_text(r4,z->name,owner->get_syscolor(col_list_text),-1,1);
	else surf.draw_text(r4,dmode_int_to_str(key,2,""),owner->get_syscolor(col_list_text_unassigned));
	r3.inset(3,0);		
}
...
{			
	...
	r2.y2 = r.get_h() - y; r2.y = r2.y2-noteh;
...
{		
	...
	{		
		...
		int subimage = 4;
		if(j<0) subimage = 13;	// drop from browser
		else if(i->playing) subimage = 9;
		else if(selected_id == j) subimage = 6;
		else if(multiselect.find(j) != multiselect.end()) subimage = 7;
		else if(hover_id == j) subimage = 5;					
		...
		{
			update_dragcorners();
			if(selected_id_corners == 8) surf.borderblit(r2,bmpdata[10]);		
			else if(selected_id_corners == 2) surf.borderblit(r2,bmpdata[11]);
		}

		r2.inset(3,0);			
		if((r2.get_h() > 8) &&(r2.get_w() > 8))surf.draw_text(r2,i->name,i->mute?owner->get_syscolor(col_standard_text_disabled):owner->get_syscolor(col_list_text));
...
{	
	...
	{		
		...
		{	
			mouse_inside = true;
			int mde = ((mode == mode_list) && rect_keybed.point_within(vg_point((int)e.x,(int)e.y)))?2:((e.buttonmask & vgm_control)?1:0);
		...
		if(e.eventtype == vget_mousemove)
		{
			...
			for(int i=0; i<n; i++)
			{
				if(zones[i].visible && t.does_intersect(zones[i].r))
				{
					...
					if(selected_id < 0) selected_id = i;					
				}
			}
			if((e.buttonmask&vgm_control)&&(multiselect.find(selected_id) == multiselect.end())) selected_id = -1;
			set_dirty();
		} 
		else if(e.eventtype == vget_mouseup)
		{
	...
	else if(controlstate == cs_drag_selection)	// selection dragging	
	{
		if(e.eventtype == vget_mousemove)
		{
			...
			if(e.buttonmask & vgm_control) owner->set_cursor(cursor_copy);
			else owner->set_cursor(cursor_move);
			...
			else diffY = (e.y - lastmouseloc.y) / noteh;
			if(diffX || diffY)
			{				
				int diff;
				if(mode == mode_pad)
		...
		} else if(e.eventtype == vget_mouseup)
		{
			...
			{			
				for(int i=0; i<n; i++)
				{
					...
					{																
						actiondata ad;
						if(e.buttonmask & vgm_control) ad.actiontype = vga_set_zone_keyspan_clone;
						else ad.actiontype = vga_set_zone_keyspan;	
						ad.id = ip_kgv_or_list;										
						ad.data.i[0] = zones[i].id;	// zone					
		...
		{			
			if(e.eventtype == vget_mousemove)
			{
				...
				if((selected_id >= 0)&&(selected_id < n))
				{
					...
					if(vdir<0) zones[selected_id].keylo = newkey;
					else if(vdir>0) zones[selected_id].keyhi = newkey;

					if(zones[selected_id].keylo > zones[selected_id].keyhi)
					{
						if(vdir<0) zones[selected_id].keyhi = zones[selected_id].keylo;
						else zones[selected_id].keylo = zones[selected_id].keyhi;
					}
	...
	{	// default
				...
				else
				{
					...
					{					
						...
						{								
							...
							if ((selected_id == i)&&(multiselect.size()<2)&&(dragcorners[2].point_within(p))) 
							{
							...
							else if ((selected_id == i)&&(multiselect.size()<2)&&(dragcorners[8].point_within(p)))
							{
		...
		{			
			if(e.activebutton == 2)
			{
				...
				if(mode == mode_list) mde.label = "Switch to pad-view";
				else mde.label = "Switch to key-view";
				...
				{
					mde.submenu = 0;	
					if(multiselect.size()>1) mde.label = "Zones";
					else mde.label = "Zone";
					mde.ad.actiontype = vga_nothing;
					md.entries.push_back(mde);

					mde.ad.actiontype = vga_deletezone;
					if(multiselect.size()>1) mde.label = "Delete selected Zones";
					else mde.label = "Delete selected Zone";
					md.entries.push_back(mde);	
					...
					mde.label = "Move to Part";	 md.entries.push_back(mde);								
					mde.submenu = &mdsub_layer;
					mde.label = "Move to Layer";	 md.entries.push_back(mde);				
					mde.submenu = 0;
		...
		{							 
			...
			for(int i=0; i<zones.size(); i++)
			{
				...
				{										
					...
					if(multiselect.find(i) != multiselect.end()) owner->set_cursor(cursor_move);					
	...
	case vga_zonelist_populate:
		{
			...
			nz.playing = false;	
			int i = zones.size();
			if(z->is_active) selected_id = i;
			if(z->is_selected) multiselect.insert(i);
			zones.push_back(nz);
		}
		break;
