...
{	
	...
	{					
		...
		{			
			...
			strlwr(temp2);			
			if(!strstr(temp,temp2)) exclude = true;
		}
		if(!exclude)
		{				
			pp_dstrect e; 				
...
{	
	...
	for(int i=scroll_pos[0]; i<crect.size(); i++)
	{
		...
		row.bound(b_upper);
		if (crect[i].invert) surf.fill_rect(row,col_bg_sel);
		row.inset(4,0);
		...
		surf.draw_text(row,categorylist->at(crect[i].category_entry_id).name,crect[i].invert?col_text_sel:col_text,-1,0);		
		if(nr++ > n_visible) break;
	}
	...
	for(i=scroll_pos[1]; i<prect.size(); i++)
	{
		...
		row.bound(b_lower);
		if (sel) surf.fill_rect(row,col_bg_sel);
		row.inset(margin2,0);				
		row.bound(b_lower);
		prect[i].r = row;
		vg_rect iconrect = row;
		...
		surf.draw_text(col2,patchlist->at(prect[i].id).column2,sel?col_text_sel:col_text,0,0);		
		if(nr++ > mNumVisiblePEntries) break;
	}
	...
	default:
		{
						...
						if(prect[i].r.point_within(p))
						{
							if(e.activebutton == 1)
							{
								if(e.buttonmask & vgm_shift)
								{
									if(entry>=0)
									{
										for(int j=min(entry,i); j<=max(entry,i); j++) multiselect.insert(j);
									}
								}
								...
								{
									if ((multiselect.find(i) == multiselect.end()) || (multiselect.size() < 2)) 
									{
										multiselect.clear();										
									}
									multiselect.insert(i);	
								...
								else if(owner->take_focus(control_id))
								{
									...
									moved_since_mdown = 0;
									if(e.buttonmask & (vgm_shift|vgm_control)) moved_since_mdown = 0xffff;	// Se till att multiselect inte töms vid shift & ctrl-klick
									owner->dropfiles.clear();
									...
									{ 
										int ee = *it;
										if((ee >= 0)&&(ee<prect.size())&&(prect[ee].id >=0)&&(prect[ee].id<patchlist->size()))	
										{										
											dropfile td;
			...
			else if(e.eventtype == vget_mousewheel)
			{
				...
				{
					if(e.eventdata[0] < 0) scroll_pos[1]+=slines;
					else scroll_pos[1]-=slines;
					set_dirty();
				} else if(b_upper.point_within(p))
				{
					if(e.eventdata[0] < 0) scroll_pos[0]+=slines;
					else scroll_pos[0]-=slines;
					set_dirty();
				}
			}
			...
			else if((e.eventtype == vget_mousedown)&&(e.activebutton == 2))
			{
				...
				mde.ad.subid = 0;								
				mde.label = "Refresh database";	 md.entries.push_back(mde);
...
void vg_browser::post_action(actiondata ad)
{
	...
	case vga_browser_category_child:
		if(categorylist->at(category).child >= 0) category = categorylist->at(category).child;
		update_lists();
		break;
	case vga_browser_category_parent:
		if(categorylist->at(category).parent >= 0) category = categorylist->at(category).parent;
		update_lists();
		break;
	case vga_browser_category_next:
		if(categorylist->at(category).sibling >= 0) category = categorylist->at(category).sibling;
		else category++;
		update_lists();
		break;
