...
void vg_menu::draw()
{
	...
	for(int c=0; c<nc; c++)
	{
		...
		{			
			...
			else if(!columns[c].menu->entries[i].submenu && (columns[c].menu->entries[i].ad.actiontype == vga_nothing))
			{
	...
	for(int i=0; i<columns.size(); i++)
	{
		...
		{			
			...
			for(int j=0; j<columns[i].menu->entries.size(); j++)
			{
				...
				{
					e = j;
					if(!columns[i].menu->entries[j].submenu && (columns[i].menu->entries[j].ad.actiontype == vga_nothing)) return false;	// non-action menues cannot be selected
					return true;
				}
			}			
			return false;
...
{	
	if(e.eventtype == vget_mousemove)
	{
		...
		if(do_update) set_dirty();	
	}	
	else if(!moved && (e.eventtype == vget_mouseup))
	{
		return; // do nothing
	}
	else if(((e.eventtype == vget_mouseup)||(e.eventtype == vget_mousedown))&&(e.activebutton == 1))
	{
		int column,entry;
		if(clicked_entry(vg_point(e.x,e.y),column,entry))
		{
...
void vg_menu::post_action(actiondata ad)
{
	...
	{				
		...
		bool do_destroy = ad.data.i[2];		// if 'true' the menudata structure should be destroyed by the reciever
		vg_menucolumn mc;
...
int vg_menu::get_n_parameters()
{
	return 0;
}
int vg_menu::get_parameter_type(int id)
{
	return 0;	
}
string vg_menu::get_parameter_name(int id)
{
	
	return "-";
}
string vg_menu::get_parameter_text(int id)
{
...
{
	style = s;
	for(int i=0; i<5; i++)	buttonbmp[i] = owner->get_art(vgct_optionmenu,style,i);	
}
...
void vg_optionmenu::draw()
{
	...
	int d = (surf.sizeY - buttonbmp[4].r.get_h()) >> 1;	// place arrow the same distance from the right edge as the top & bottom edges
	...
	r.inset(5,0);
	if(!disabled) surf.draw_text(r, label, owner->get_syscolor(col_standard_text));
}
...
void vg_optionmenu::processevent(vg_controlevent &e)
{
	...
	else if(((e.eventtype == vget_mousedown)&&(e.activebutton == 1))||(e.eventtype == vget_hotkey))
	{									
		//down = false;
		actiondata ad;
		ad.id = control_id;
...
void vg_optionmenu::post_action(actiondata ad)
{
	...
	else if(ad.actiontype == vga_entry_add_ival_from_self)
	{
		...
		if(offset) mde.ad.subid += offset_amount * owner->filters[offset];
	...
	else if(ad.actiontype == vga_entry_setactive)
	{
		int i = ad.data.i[0];
		value_int = i;
		if((i>=0)&&(i<md.entries.size())) label = md.entries[i].label;		
		set_dirty();
	}	
...
int vg_optionmenu::get_n_parameters()
{
	return 6;
}
