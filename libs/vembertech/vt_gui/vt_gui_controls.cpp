...
std::string dmode_float_to_str(float x, int dmode, std::string unit)
{
	...
	if (unit.empty()) noprefix = true;
	if(!unit.compare("dB")) noprefix = true;
	if(!unit.compare("c")) noprefix = true;
	...
	else if(!noprefix && (ax>=1000)) sprintf(ch,"%.2f",x*0.001f);
	else if(!noprefix && (ax<0.01f)) sprintf(ch,"%.1f",x*1000.f);	
	else sprintf(ch,"%.2f",x);
...
{
	style = s;
	for(int i=0; i<4; i++)	buttonbmp[i] = owner->get_art(vgct_button,style,i);	
}
...
void vg_button::draw()
{
	...
	else if((action_i == vga_intval) && action_p[0]) idx = 2;
	else if(down) idx = 2;
	else if(hover) idx = 1;	
	
	surf.borderblit(vg_rect(0,0,surf.sizeX,surf.sizeY),buttonbmp[idx]);	
	surf.draw_text(vg_rect(0,0,surf.sizeX,surf.sizeY),decode_vars(label),disabled?owner->get_syscolor(col_standard_text_disabled):
		(down?owner->get_syscolor(col_standard_text):owner->get_syscolor(col_standard_text)));
	draw_needed = false;
}
...
void vg_button::processevent(vg_controlevent &e)
{
	...
	if(((e.eventtype == vget_mousedown) || (e.eventtype == vget_mousedblclick))&&(e.activebutton == 1)) down = true;
	else if(((e.eventtype == vget_mouseup)&&(e.activebutton == 1)&&down)||(e.eventtype == vget_hotkey))
	{		
		if(action_i == vga_intval) action_p[0] = action_p[0]?0:1;
		actiondata ad;		
		ad.data.i[0] = action_p[0];
		ad.data.i[1] = action_p[1];
		ad.actiontype = action_i;
		if((action_i == vga_exec_external)||(action_i == vga_url_external)||(action_i == vga_doc_external))			
		{
	...
	else if(e.eventtype == vget_mouseenter) hover = true;
	else if(e.eventtype == vget_mouseleave)
	{ 
		down = false; hover = false; 
	}
	else return;	// skip update
...
int vg_button::get_n_parameters()
{
	return 7;
}
...
void vg_button::set_parameter_text(int id,std::string text, bool no_refresh)
{
	switch(id)
	{
		...
		case 3:
			{
				action = text;
				if(action.compare("filter")==0) action_i = vga_filter;
				else if(action.compare("int")==0) action_i = vga_intval;
				else if(action.compare("exec")==0) action_i = vga_exec_external;
				else if(action.compare("url")==0) action_i = vga_url_external;
				else if(action.compare("doc")==0) action_i = vga_doc_external;
				else if(action.compare("savepatch")==0) action_i = vga_save_patch;
				else if(action.compare("savemulti")==0) action_i = vga_save_multi;
				else action_i = vga_click;				
			}
			break;
		case 4:			
		case 5:	
...
{	
	...
	{		
		...
		for(int i=0; i<sub_buttons; i++)
		{
			...
			if(sel_id == i) idx = 2;
			else if(hover_id == i) idx = 1;

			surf.borderblit(r,buttonbmp[idx]);	
			surf.draw_text(r,entry,disabled?owner->get_syscolor(col_standard_text_disabled):
			((idx==2)?owner->get_syscolor(col_standard_text):owner->get_syscolor(col_standard_text)));
			if(b==label.size()) break;
			a=b+1;
		}
	}
}
...
void vg_multibutton::processevent(vg_controlevent &e)
{
	...
	if((e.eventtype == vget_mousedown)||(e.eventtype == vget_mousedblclick)&&(e.activebutton == 1)) 
	{
		int x = max(0,min(surf.sizeX,e.x));
		int y = max(0,min(surf.sizeY,e.y));
		int n_id = max(0,min(sub_columns-1,(x/sb_width))) + sub_columns*max(0,min(sub_rows-1,(y/sb_height)));
	...
	else if((e.eventtype == vget_mouseenter)||(e.eventtype == vget_mousemove))
	{
		...
		if(e.buttonmask & vgm_LMB) sel_id = hover_id;
	}
	else if(e.eventtype == vget_mouseleave)
	{ 
		hover_id = -1; 		
...
void vg_slider::draw()
{
	...
	if(disabled) c0 = (c0 & 0x00ffffff) | (((c0&0xff000000) >> 1) & 0xff000000);
	if(disabled) c0t = ((col[0] & (0x00fefefe)) + 0x00404040) | 0xff000000;
	if(disabled) c1 = (c1 & 0x00ffffff) | (((c1&0xff000000) >> 1) & 0xff000000);
	...
	if(label.size())
	{
		...
		else
		{
			...
			char text[64];
			if(temposync) sprintf_s(text,sizeof(text),"%s",temposynced_float_to_str(value_float).c_str());
			else if(floatmode) sprintf_s(text,sizeof(text),"%s",dmode_float_to_str(value_float,dmode,unit).c_str());
			else sprintf_s(text,sizeof(text),"%s",dmode_int_to_str(value_int,dmode,unit).c_str());
			if(!disabled) surf.draw_text(labelrect,text,c0t,1,-1);
		}		
	}
	...
	{
		vg_rect rd = sr_outer;
		if(vertical) rd.inset((sr_outer.get_w()-1)>>1,margin);
		else rd.inset(margin,(sr_outer.get_h()-1)>>1);
		surf.fill_rect(rd,c0,false);
		
		bool bp_tray;	// if true draw bipolar tray
		if(floatmode) bp_tray = fabs(f_max + f_min) < (0.1*f_max);
		else bp_tray = (i_max + i_min) == 0;
	...
	if(!disabled)
	{
		...
		if(hover || down) surf.borderblit(headrect,head[vertical?1:0][4]);	
	}				
}
...
void vg_slider::processevent(vg_controlevent &e)
{
	...
	if(e.eventtype == vget_mousewheel)
	{
		...
		{						
			...
			if(temposync) ad.data.f[0] = temposync_quantitize(ad.data.f[0]);
			owner->post_action(ad,this);
		}
		else
		{			
	...
	else if(((e.eventtype == vget_mousedown)||(e.eventtype == vget_mousedblclick))&&(e.activebutton == 1)) 
	{
	...
	{		
		...
		if(e.buttonmask & vgm_RMB) delta *= 0.1f;
		if(e.buttonmask & vgm_shift) delta *= 0.1f;		
	...
	{
		down = false;
		owner->move_mousepointer(location.x + headrect.get_mid_x(), location.y + headrect.get_mid_y());	
...
int vg_slider::get_n_parameters()
{
	return 6;
}
...
{
	style = s;
	for(int i=0; i<4; i++)	buttonbmp[i] = owner->get_art(vgct_textedit,style,i);	
}
...
void vg_textedit::draw()
{
	...
	{				
		...
		surf.draw_text(vg_rect(xsplit + padding,0,surf.sizeX-padding,surf.sizeY),text,disabled?owner->get_syscolor(col_standard_text_disabled):owner->get_syscolor(col_standard_text),0,0,down?cursor:-1, down);
		surf.draw_text(vg_rect(0,0,xsplit,surf.sizeY),label,disabled?owner->get_syscolor(col_standard_text_disabled):owner->get_syscolor(col_standard_text));
	}
	else
	{
		surf.borderblit(vg_rect(0,0,surf.sizeX,surf.sizeY),buttonbmp[idx]);		
		surf.draw_text(vg_rect(padding,0,surf.sizeX-padding,surf.sizeY),text,disabled?owner->get_syscolor(col_standard_text_disabled):owner->get_syscolor(col_standard_text),0,0,down?cursor:-1, down);
	}	
}
...
void vg_textedit::processevent(vg_controlevent &e)
{
	...
	else if(((e.eventtype == vget_mousedown)||(e.eventtype == vget_mousedblclick))&&(e.activebutton == 2)) 
	{
		text.clear();
		cursor = 0;	
		actiondata ad;		
	...
	else if(down && (e.eventtype == vget_character))
	{
		if(!(e.eventdata[0]&0xffffff00)) 
		{
			...
			case 0:
				break;
			case 0x8:	// backspace
				if(cursor && (text.size() > 0)) text.erase(text.begin()+cursor-1);
				cursor--;
				break;
			default:
				//text += c;
	...
	{		
		...
		case VK_RETURN:						
			...
			break;
		case VK_DELETE:
			if((cursor<text.size()) && (text.size() > 0)) text.erase(text.begin()+cursor);			
			break;
		case VK_LEFT:
	...
	{		
		if(!((e.x<0.f)||(e.x > location.get_w())||(e.y<0.f)||(e.y > location.get_h()))) owner->set_cursor(cursor_text);
	}
	else if(e.eventtype == vget_mouseleave) 
	{
		hover = false;
...
int vg_textedit::get_n_parameters()
{
	return 2;
}
...
void vg_paramedit::draw()
{
	...
	int idx = 0;	
	if(disabled) idx = 3;	
	else if(down||textedit) idx = 2;
	else if(hover) idx = 1;	
	
	int padding = 4;	

	char txt[64];	
	if(textedit) sprintf(txt,"%s",text.c_str());
	else if(floatmode) sprintf(txt,"%s",dmode_float_to_str(value_float,dmode,unit).c_str());
	else sprintf(txt,"%s",dmode_int_to_str(value_int,dmode,unit).c_str());		
	...
	{	
		...
		surf.draw_text(labelrect,label,disabled?owner->get_syscolor(col_standard_text_disabled):owner->get_syscolor(col_standard_text));
	}
	surf.borderblit(textrect,buttonbmp[idx]);		
	textrect.inset(padding,0);
	if(!disabled) surf.draw_text(textrect,txt,owner->get_syscolor(col_standard_text),0,0,textedit?cursor:-1);	
}
...
void vg_paramedit::processevent(vg_controlevent &e)
{
	...
	else if((e.eventtype == vget_mousedblclick)&&(e.activebutton == 1))
	{
		...
		down = false;
		textedit = true;
		if(floatmode) text = dmode_float_to_str(value_float,dmode,"");
		else text = dmode_int_to_str(value_float,dmode,"");
		cursor = text.size();
		owner->take_focus(control_id);	
	}
	else if(down&&(e.eventtype == vget_mousemove)&&(e.buttonmask & 1))
	{		
		...
		if(e.buttonmask & vgm_RMB) delta *= 0.05f;
		if(e.buttonmask & vgm_shift) delta *= 0.05f;
	...
	{
		down = false;
		//owner->move_mousepointer(location.x + textrect.get_mid_x(), location.y + textrect.get_mid_y());	
		owner->move_mousepointer(location.x + lastmouseloc.x, location.y + lastmouseloc.y);	
		owner->show_mousepointer(true);
		owner->drop_focus(control_id);
...
int vg_paramedit::get_n_parameters()
{
	return 5;
}
...
void vg_textview::set_style(int s)
{
	style = s;	
}
...
void vg_textview::draw()
{
	...
	surf.draw_text_multiline(vg_rect(margin,0,surf.sizeX-margin,surf.sizeY),textdata,owner->get_syscolor(col_standard_text),0,-1);	
}
...
int vg_textview::get_n_parameters()
{
	return 2;
}
...
void vg_VU::draw()
{
	...
	for(int i=0; i<NumChannels; i++)
	{
		...
		if(vu.stereo)
		{
			...
			r1.inset(1,0);
			r2.inset(1,0);
			if(vu.clip1) surf.fill_rect(r1,owner->get_syscolor(col_vu_overload));
			else
			{
			...
			if(vu.clip2) surf.fill_rect(r2,owner->get_syscolor(col_vu_overload));
			else
			{
