...
public:
	...
	void draw_text(vg_rect r, string text, unsigned int color, int halign=0, int valign=0, int ibeampos=-1, bool IgnoreParsing=false);	
	void draw_text_multiline(vg_rect r, string text, unsigned int color, int halign=0, int valign=0);	
...
public:	
	...
	virtual string serialize_tag(){ return "none"; } // defining the tags in two different places is somewhat incoherent
	...
	void setlabel(string lbl){ label = lbl; /*draw(); set_dirty();*/ }
	...
	virtual string get_parameter_name(int id){ return ""; }
	virtual string get_parameter_text(int id){ return ""; }
	virtual void set_parameter_text(int id,string text,bool no_refresh=false){return;}
...
public:
	...
	bool create(string filename, bool is_toolbox=false, vg_window* owner=0, void *syswindow=0);
	void destroy();
	...
	vector<int> p2l_map;	// map external "interaction parameters" to a control id
	...
	virtual string param_get_label_from_id(int id){return "-";}
	virtual int param_get_id_from_label(string s){return -1;}
	...
	bool dialog_confirm(wstring label, wstring text, int buttonmode);
	wstring dialog_save(wstring label, wstring startdir, int type);
	void dialog_save_part();
	void dialog_save_multi();

	vg_rect get_rect();
	vg_surface surf,background,altbg[8];
	string background_filename,altbg_filename[8];
	vector<vg_control*> children;
	multimap<int,vg_control*> children_by_pid;
	set<int> dirtykids;	// add to list when dirty, remove when redrawn
	...
	vector<vg_surface*> artdata_loaded;
	vg_font* get_font(unsigned int id);
	void set_cursor(int);
	TiXmlDocument artdoc;
	vector<vg_surface*> artsurfs;
	vector<vg_font*> artfonts;
	int filters[max_vgwindow_filters];
	HWND syswindow;
	bool ignore_next_mousemove;
	vector<dropfile> dropfiles;
	wstring savepart_fname;
...
protected:			
	...
	HHOOK kbdhook;
	BITMAPINFO bmpinfo;	
	string rootdir_skin,rootdir_default;
	bool editmode;
